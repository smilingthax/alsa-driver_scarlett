...
typedef struct _snd_pcm_link {		/* keep linked substreams */
	spinlock_t lock;
...
struct _snd_pcm_substream {
	...
	snd_pcm_link_t local_link;	/* linked list manager */
	snd_pcm_link_t *link;		/* pointer to actual linked list manager */
	/* -- assigned files -- */
	snd_pcm_file_t *file;
	struct file *ffile;
#if defined(CONFIG_SND_PCM_OSS) || defined(CONFIG_SND_PCM_OSS_MODULE)
...
#define snd_pcm_for_each_streams_entry(pos) \
