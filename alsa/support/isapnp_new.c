...
__initfunc(static int isapnp_next_rdp(int rdp))
{
...
static int isapnp_config_activate(struct isapnp_dev *dev)
{
	...
	for (tmp = 0; tmp < 8 && dev->resource[tmp].start != DEVICE_IO_NOTSET; tmp++)
		isapnp_cfg_set_word(ISAPNP_CFG_PORT+(tmp<<1), dev->resource[tmp].start);
	if (dev->irq != DEVICE_IRQ_NOTSET) {
		if (dev->irq == 2)
			dev->irq = 9;
		isapnp_cfg_set_byte(ISAPNP_CFG_IRQ+(0<<1), dev->irq);
	}
	if (dev->irq2 != DEVICE_IRQ_NOTSET) {
		if (dev->irq2 == 2)
			dev->irq2 = 9;
		isapnp_cfg_set_byte(ISAPNP_CFG_IRQ+(1<<1), dev->irq2);
	}
	for (tmp = 0; tmp < 2 && dev->dma[tmp] != DEVICE_DMA_NOTSET; tmp++)
		isapnp_cfg_set_byte(ISAPNP_CFG_DMA+tmp, dev->dma[tmp]);
	for (tmp = 0; tmp < 4 && dev->resource[tmp].start != DEVICE_IO_NOTSET; tmp++)
		isapnp_cfg_set_word(ISAPNP_CFG_MEM+(tmp<<2), (dev->resource[tmp + 8].start >> 8) & 0xffff);
