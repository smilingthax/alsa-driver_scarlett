 ...
 *
 * Changes:
 * 19980227   Takashi Iwai   Arrays to linked lists + sample write changes.
 */
...
typedef struct snd_sf_sample {
	...
	int counter;
	int mem_offset;	/* zero-based memory offset - callback function
			 * must rewrite this value
			 */
...
typedef struct snd_sf_list {
	int mem_offset;	/* zero based offset into sample memory */
...
int snd_soundfont_mem_offset(snd_sf_list_t *sflist);
