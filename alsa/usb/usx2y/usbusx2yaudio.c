...
static void i_usX2Y_urb_play_complete__old_(struct urb *urb);
static void i_usX2Y_urb_capt_complete__old_(struct urb *urb);
#endif
 ...
 */
inline static unsigned usX2Y_get_usb_rate(unsigned int rate)
{
 ...
 */
static int usX2Y_urb_capt_prepare(snd_usX2Y_substream_t *subs,
 ...
 */
static void usX2Y_framecpy_4c_16(unsigned char *p_dma_area, unsigned char *cp, int cnt)
{
	do{
...
static void usX2Y_framecpy_4c_24(unsigned char *p_dma_area, unsigned char *cp, int cnt)
{
	do{
			      ...
			      struct urb *urb)
{
	...
	if (runtime->channels == 4)
		fourc_cpy = usX2Y->stride == 4 ? usX2Y_framecpy_4c_16 : usX2Y_framecpy_4c_24;
 ...
 */
inline static int usX2Y_urb_play_retire(snd_usX2Y_substream_t *subs,
			       snd_pcm_runtime_t *runtime,
			       struct urb *urb)
{
	unsigned long	flags;
...
inline static int usX2Y_urb_submit(snd_usX2Y_substream_t *subs, struct urb *urb)
{
	int err;
 ...
 */
static void usX2Y_urb_play_complete_(struct urb* urb)
{
	...
	if (subs->running && usX2Y_urb_play_retire(subs, substream->runtime, urb))
		return;
	if (! subs->running) /* can be stopped during retire callback */
		return;
	if (	(err = usX2Y_urb_play_prepare(subs, substream->runtime, urb)) < 0
	    ||	(err = usX2Y_urb_submit(subs, urb)) < 0
		) {
...
static void i_usX2Y_urb_play_complete(struct urb* urb, struct pt_regs *regs)
{
	snd_usX2Y_substream_t *subs = (snd_usX2Y_substream_t*)urb->context;
	...
	usX2Y_urb_play_complete_(urb);
}
/*
 * complete callback from data urb
 */
static void i_usX2Y_urb_capt_complete(struct urb* urb, struct pt_regs *regs)
{
	...
	if (captsubs->bussing && usX2Y_urb_capt_retire(captsubs, runtime, urb))
		return;
 	if (! captsubs->bussing) /* can be stopped during retire callback */
		return;
	if ((err = usX2Y_urb_capt_prepare(captsubs, runtime, urb)) < 0 ||
	    (err = usX2Y_urb_submit(captsubs, urb)) < 0) {
		snd_printd(KERN_ERR "cannot submit urb (err = %d)\n", err);
		if (pcm_captsubs)
	...
	if (urb->pipe == captsubs->datapipe[0] && 
		...
		usX2Y_urb_play_complete_(captsubs->stream->usX2Y->play_urb_waiting[0]);
 ...
 */
static int usX2Y_urbs_deactivate(snd_usX2Y_substream_t *subs)
{
	int i, alive, ep;
...
static int usX2Y_urb_start(snd_usX2Y_substream_t *subs)
{
	int i, err, ep;
 ...
 */
static int usX2Y_urb_capt_start(snd_usX2Y_substream_t *subs, snd_pcm_runtime_t *runtime)
{
	int i;
	...
	if (! subs->bussing) {
		...
		for (ep = 0; ep < subs->endpoints; ep++)
			for (i = 0; i < NRURBS; i++) {
				...
				if (usX2Y_urb_capt_prepare(subs, runtime, subs->dataurb[ep][i]) < 0) {
 ...
 __error:
	...
	usX2Y_urbs_deactivate(subs);
	return -EPIPE;
}
/*
 * set up and start data/sync urbs
 */
static int usX2Y_urb_play_start(snd_usX2Y_substream_t *subs, snd_pcm_runtime_t *runtime)
{
	int i;
	usX2Ydev_t* usX2Y = subs->stream->usX2Y;
...
#endif
		if (usX2Y_urb_play_prepare(subs, runtime, subs->dataurb[0][i]) < 0) {
			snd_printk(KERN_ERR "cannot prepare datapipe for urb %d\n", i);
			goto __error;
		}
	}
...

	{
		...
		if (! capsubs->bussing) {
			...
			usX2Y_set_format(capsubs, runtime);
			usX2Y_substream_prepare(capsubs, runtime);
			for (ep = 0; ep < capsubs->endpoints; ep++)
				for (i = 0; i < NRURBS; i++) {
					snd_assert(capsubs->dataurb[ep][i], return -EINVAL);
					if (usX2Y_urb_capt_prepare(capsubs, runtime, capsubs->dataurb[ep][i]) < 0) {
						snd_printk(KERN_ERR "cannot prepare datapipe for urb %d\n", i);
						goto __error;
					}
				}
			capsubs->remainder = 0;
			if (usX2Y_urb_start(capsubs))
				goto __error;
		}
	}
...
static int snd_usX2Y_pcm_capt_trigger(snd_pcm_substream_t *substream, int cmd)
{
	...
	case SNDRV_PCM_TRIGGER_START:
		...
		err = usX2Y_urb_capt_start(subs, substream->runtime);
		break;
	case SNDRV_PCM_TRIGGER_STOP:
		snd_printd("snd_usX2Y_pcm_capt_trigger(STOP)\n");
		err = usX2Y_urbs_deactivate(subs);
		break;
	default:
		err = -EINVAL;
		break;
...
static int snd_usX2Y_pcm_play_trigger(snd_pcm_substream_t *substream, int cmd)
{
	...
	case SNDRV_PCM_TRIGGER_START:
		...
		err = usX2Y_urb_play_start(subs, substream->runtime);
		break;
	case SNDRV_PCM_TRIGGER_STOP:
 ...
 * release a urb data
 */
static void usX2Y_release_urb_ctx(struct urb** urb, int free_tb)
{
	if (*urb) {
		if (free_tb)
...
static void usX2Y_urbs_release(snd_usX2Y_substream_t *subs)
{
	...
	if (usX2Y_urbs_deactivate(subs) > 0)
		snd_printd("waitclear\n");
	usX2Y_urbs_wait_clear(subs);
	
	for (ep = 0; ep < subs->endpoints; ep++)
		for (i = 0; i < NRURBS; i++)
			usX2Y_release_urb_ctx(subs->dataurb[ep] + i, subs == (subs->stream->substream + SNDRV_PCM_STREAM_CAPTURE));
...
static int usX2Y_substream_prepare(snd_usX2Y_substream_t *subs, snd_pcm_runtime_t *runtime)
{
	...
	subs->freqn = subs->freqm = usX2Y_get_usb_rate(runtime->rate);
		...
		for (i = 0; i < NRURBS; i++) {
			...
			*purb = usb_alloc_urb(NRPACKS, GFP_KERNEL);
			...
			(*purb)->complete = is_playback ?
				snd_usb_complete_callback(i_usX2Y_urb_play_complete) :
				snd_usb_complete_callback(i_usX2Y_urb_capt_complete);
		}
		...
		do {
			...
			for (i = 0; i < NOOF_SETRATE_URBS; ++i) {
				...
				usb_fill_bulk_urb(us->urb[i], usX2Y_stream->usX2Y->chip.dev, usb_sndbulkpipe(usX2Y_stream->usX2Y->chip.dev, 4),
						  usbdata + i, 2, i_usX2Y_04Int, usX2Y_stream->usX2Y);
#ifdef OLD_USB
 ...
 * free the buffer
 */
static int snd_usX2Y_hw_free(snd_pcm_substream_t *substream)
{
...
static int snd_usX2Y_pcm_prepare(snd_pcm_substream_t *substream)
{
	...
	if (! subs->bussing)	
		usX2Y_urbs_release(subs);
	if ((err = usX2Y_set_format(subs, runtime)) < 0)
		return err;

	return usX2Y_substream_prepare(subs, runtime);
}
...
static int usX2Y_pcm_open(snd_pcm_substream_t *substream, int direction,
...
static int snd_usX2Y_playback_open(snd_pcm_substream_t *substream)
{
	snd_printd("snd_usb_playback_open()\n");
	return usX2Y_pcm_open(substream, SNDRV_PCM_STREAM_PLAYBACK, &snd_usX2Y_2c);
}

static int snd_usX2Y_playback_close(snd_pcm_substream_t *substream)
{
	snd_usX2Y_stream_t	*as = snd_pcm_substream_chip(substream);
...
static int snd_usX2Y_capture_open(snd_pcm_substream_t *substream)
{
	snd_usX2Y_stream_t *as = snd_pcm_substream_chip(substream);
	snd_pcm_hardware_t *hardware = as->usX2Y->chip.dev->descriptor.idProduct == USB_ID_US428 ? &snd_usX2Y_4c : &snd_usX2Y_2c;
	return usX2Y_pcm_open(substream, SNDRV_PCM_STREAM_CAPTURE, hardware);
}

static int snd_usX2Y_capture_close(snd_pcm_substream_t *substream)
{
	int err = 0;
	snd_usX2Y_stream_t *as = snd_pcm_substream_chip(substream);
...
static snd_pcm_ops_t snd_usX2Y_playback_ops = 
{
	.open =		snd_usX2Y_playback_open,
	.close =	snd_usX2Y_playback_close,
	.ioctl =	snd_pcm_lib_ioctl,
	.hw_params =	snd_usX2Y_hw_params,
	.hw_free =	snd_usX2Y_hw_free,
	.prepare =	snd_usX2Y_pcm_prepare,
	.trigger =	snd_usX2Y_pcm_play_trigger,
	.pointer =	snd_usX2Y_pcm_pointer,
};

static snd_pcm_ops_t snd_usX2Y_capture_ops = 
{
	.open =		snd_usX2Y_capture_open,
	.close =	snd_usX2Y_capture_close,
	.ioctl =	snd_pcm_lib_ioctl,
	.hw_params =	snd_usX2Y_hw_params,
	.hw_free =	snd_usX2Y_hw_free,
	.prepare =	snd_usX2Y_pcm_prepare,
 ...
 */
static snd_usX2Y_substream_t *usX2Y_substream_init(snd_usX2Y_stream_t *stream, enum sndrv_pcm_stream  dir)
{
 ...
 * free a substream
 */
static void usX2Y_substream_free(snd_usX2Y_substream_t *subs)
{
}
 ...
 */
static void usX2Y_audio_stream_free(snd_usX2Y_stream_t *stream)
{
...
static void snd_usX2Y_audio_pcm_free(snd_pcm_t *pcm)
{
	...
	if (stream) {
		...
		usX2Y_audio_stream_free(stream);
	}
}

static int usX2Y_audio_stream_new(snd_card_t* card)
{
	...
	usX2Y_substream_init(usX2Y_stream, SNDRV_PCM_STREAM_PLAYBACK);
	usX2Y_capt_substream = usX2Y_substream_init(usX2Y_stream, SNDRV_PCM_STREAM_CAPTURE);
	...
	snd_pcm_set_ops(pcm, SNDRV_PCM_STREAM_PLAYBACK, &snd_usX2Y_playback_ops);
	snd_pcm_set_ops(pcm, SNDRV_PCM_STREAM_CAPTURE, &snd_usX2Y_capture_ops);
...
//		snd_usX2Y_audio_free(usX2Y(card));
	...
	err = usX2Y_audio_stream_new(card);
...
static void i_usX2Y_urb_play_complete__old_(struct urb *urb)
{
	i_usX2Y_urb_play_complete(urb, NULL);
}

static void i_usX2Y_urb_capt_complete__old_(struct urb *urb)
{
	i_usX2Y_urb_capt_complete(urb, NULL);
}
#endif
