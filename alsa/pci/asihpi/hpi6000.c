...
)
{
	...
	case HPI_CONTROL_SET_STATE:
		HW_Message(pao, phm, phr);
		SyncControlCache(&((struct hpi_hw_obj *)pao->priv)->
...
/* Was 200000. */
/* Increased to 2000000 for ASI8801 so we don't get 938 errors. */
		while (dwTimeout) {
...
/* write to Debug register which is 24bits wide (D31-D8) */
...
/* errors fatal to this interface module */
	if ((phr->wError >= HPI6000_ERROR_BASE) && (phr->wError <= 2000)) {
		HPI_DEBUG_LOG(ERROR,
			"HPI Response - error# %d, adapter# %d, "
			"function# 0x%x\n",
			phr->wError, phm->wAdapterIndex, phm->wFunction);
		if (hpiDebugLevel >= HPI_DEBUG_LEVEL_ERROR)
			hpi_debug_data((u16 *)phm,
				sizeof(struct hpi_message) / sizeof(u16));
	}
