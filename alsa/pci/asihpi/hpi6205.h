...
{
	HW32 dwSamplesProcessed;
	HW32 dwAuxiliaryDataAvailable;
	HW32 dwStreamState;
	HW32 dwDSPIndex;		// DSP index in to the host bus master buffer.
	HW32 dwHostIndex;		// Host index in to the host bus master buffer.
	HW32 dwSizeInBytes;
...
typedef volatile struct
{
	HW32 dwNumberOfControls;
	HW32 dwPhysicalPCI32address;
	HW32 dwSpare;
} H620_CONTROLCACHE;
...
typedef volatile struct {
	HW32 dwHostCmd;
	HW32 dwDspAck;
	HW32 dwTransferSizeInBytes;
	union {
		HPI_MESSAGE MessageBuffer;
		HPI_RESPONSE ResponseBuffer;
		HW8 bData[HPI6205_SIZEOF_DATA];
	} u;
