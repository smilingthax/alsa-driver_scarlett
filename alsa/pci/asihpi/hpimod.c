...
if (in_interrupt()) printk("in interrupt ");
if (in_softirq()) printk("in softirq ");
if (in_irq()) printk("in irq ");
if (in_atomic()) printk("in atomic ");
if (irqs_disabled()) printk("interrupts disabled ");
printk("\n");
#endif
...
static long hpi_ioctl(struct file *file,
...
#endif
{
	...
	/* get_user(hm.wSize, (HW16 __user *)phm); */
	uncopied_bytes=copy_from_user(&hm, phm, sizeof(hm));
	if (uncopied_bytes)
		return -EFAULT;

	if ( hm.wAdapterIndex > HPI_MAX_ADAPTERS ) {
		HPI_InitResponse(&hr, HPI_OBJ_ADAPTER, HPI_ADAPTER_OPEN,HPI_ERROR_BAD_ADAPTER_NUMBER );
	...
	// Response gets filled in either by copy from cache, or by HPI_Message()
	{
		...
		case HPI_ISTREAM_READ:
			...
			ptr = (HW16 __user *) hm.u.d.u.Data.pbData;
		...
		if ((nAdapter >= HPI_MAX_ADAPTERS || nAdapter < 0) &&
			...
			if (wrflag == 0) {
				...
				uncopied_bytes = copy_from_user(pa->pBuffer, ptr, size);
				if (uncopied_bytes) {
					HPI_DEBUG_LOG2(WARNING,
...
static int __devinit adapter_probe(struct pci_dev *pci_dev,
	...
	HPI_InitResponse( &hr, HPI_OBJ_SUBSYSTEM, HPI_SUBSYS_CREATE_ADAPTER, HPI_ERROR_PROCESSING_MESSAGE );
	...
	for( idx = 0; idx < HPI_MAX_ADAPTER_MEM_SPACES; idx++ ) {
		HPI_DEBUG_LOG4(DEBUG,"Resource %d %s %lx-%lx\n", idx, pci_dev->resource[idx].name,
			(unsigned long)pci_dev->resource[idx].start, (unsigned long)pci_dev->resource[idx].end);

		memlen = pci_resource_len( pci_dev, idx );
		if ( memlen ) {
			adapter.apRemappedMemBase[idx] = ioremap(pci_resource_start(pci_dev,idx), memlen);
			if (!adapter.apRemappedMemBase[idx]) {
				HPI_DEBUG_LOG0(ERROR,"ioremap failed, aborting\n");
				//unmap previously mapped pci mem space
				goto err;
			}
		} else adapter.apRemappedMemBase[idx] = NULL;
	...
	Pci.wVendorId = (HW16)pci_dev->vendor;
	Pci.wDeviceId = (HW16)pci_dev->device;
	Pci.wSubSysVendorId = (HW16)(pci_dev->subsystem_vendor & 0xffff);
	Pci.wSubSysDeviceId = (HW16)(pci_dev->subsystem_device & 0xffff);
	...
	if ((adapter.pBuffer = vmalloc(bufsize)) == NULL) {
		HPI_DEBUG_LOG1(ERROR,"HPI could not allocate kernel buffer size %d\n", bufsize);
		goto err;
	}
#endif
	...
	if ( hr.wError == 0 ) {
		...
		printk(KERN_INFO "Probe found adapter ASI%04X HPI index #%d.\n",
		       hr.u.s.awAdapterList[hr.u.s.wAdapterIndex], hr.u.s.wAdapterIndex );
...
#ifdef ALSA_BUILD
		if ( snd_asihpi_bind( &adapters[hr.u.s.wAdapterIndex] ) ) goto err;
#endif
...
static void __devexit adapter_remove(struct pci_dev *pci_dev)
{
	...
	printk(KERN_INFO "PCI device (%04x:%04x,%04x:%04x,%04x), HPI index # %d, removed.\n",
			pci_dev->vendor, pci_dev->device, pci_dev->subsystem_vendor,
...
static int __init hpimod_init(void) {
	...
	printk(KERN_INFO "ASIHPI driver %s debug=%d ", __stringify(DRIVER_VERSION), hpiDebugLevel);
