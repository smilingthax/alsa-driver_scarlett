...
/**************************** LOCAL PROTOTYPES **************************/
static short    Dpi_Command(HW32 dw56301Base, HW32 Cmd);
...
short Hpi56301_CheckAdapterPresent( HW32 dwMemBase )
{
    ...
    if(dwData != 0x1DBFEL )
    {
        HPI_PRINT_ERROR("Read should be 1DBFE, but is %lX\n",
                        dwData);
        return(1);      //error
    }
    else
    {
        ...
        HPI_PRINT_DEBUG("PCI registers, HCTR=%08lX HSTR=%08lX HCVR=%08lX\n",
...
{
	 HW32 dwMemBase=pao->Pci.dwMemBase[0];
    DSP_CODE DspCode;
    short nError=0;
...
/************************************************************************/
static HW16 Hpi56301_Resync(HW32 dwBase)
{
    HW16 wError;
    //?	 HW32 dwData;
    HW32 dwTimeout;
    ...
    dwTimeout=TIMEOUT;
    while (dwTimeout && (HPIOS_MEMREAD32(dwBase+REG56301_HSTR) & HSTR_HRRQ) )
    {
                    ...
                    short wDataType )
{
    ...
    HPI_PRINT_VERBOSE("Send, ptr = 0x%lx, len = %ld, cmd = %d\n",
                    (HW32)pData, dwLengthInWords, wDataType);
                   ...
                   short wDataType)
{
    ...
    HPI_PRINT_VERBOSE("Get, ptr = 0x%lx, cmd = %d\n", (HW32)pData, wDataType);
    ...
    *pdwLengthInWords=dwLength1+dwLength2;		// AGE Feb 19 98
    ...
    {
        HPI_PRINT_ERROR("Unexpected data wrap for response, got %lu\n", dwAfterWrap);
...
/**************************** LOCAL FUNCTIONS ***************************/
/************************************************************************/
void Dpi_SetFlags(HW32 dw56301Base, short Flagval)
{
    HW32 dwHCTR;
...
short Dpi_WaitFlags(HW32 dw56301Base, short Flagval)
{
    HW16 wFlags;
...
/************************************************************************/
short Dpi_GetFlags(HW32 dw56301Base)
{
    HW32 dwHSTR1,dwHSTR2;
...
/************************************************************************/
short Dpi_Command(HW32 dw56301Base, HW32 Cmd)
{
    ...
    HPI_PRINT_VERBOSE("(cmd = %ld) attempts = %ld\n",
...
short DpiData_Read32(HW32 dwBase, HW32 HUGE *pdwData)
{
    ...
    {
        HPI_PRINT_ERROR("DpiData_Read32() attempts = %ld\n", TIMEOUT - dwTimeout);
    }
