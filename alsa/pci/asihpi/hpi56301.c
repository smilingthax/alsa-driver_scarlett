...
#ifdef HPI_OS_WDM
NTSTATUS ASIWDM_WriteLogEntry (char *pBuffer);
NTSTATUS ASIWDM_LogMessage (char *pBuffer);

//#define LOG_REGISTER_READS_AND_WRITES
...
void LogRegisterRead(HW32 offset, HW32 value, int error)
{
	switch(offset)
	{
	case 0x10:	RegLog[RegLogIndex].pReg = szHCTR;	break;
	case 0x14:	RegLog[RegLogIndex].pReg = szHSTR;	break;
	case 0x18:	RegLog[RegLogIndex].pReg = szHCVR;	break;
	case 0x1C:	RegLog[RegLogIndex].pReg = szHRXM;	break;
	default:	RegLog[RegLogIndex].pReg = szUNKN;
	}
...
void LogRegisterWrite(HW32 offset, HW32 value)
{
	switch(offset)
	{
	case 0x10:	RegLog[RegLogIndex].pReg = szHCTR;	break;
	case 0x14:	RegLog[RegLogIndex].pReg = szHSTR;	break;
	case 0x18:	RegLog[RegLogIndex].pReg = szHCVR;	break;
	case 0x1C:	RegLog[RegLogIndex].pReg = szHTXR;	break;
	default:	RegLog[RegLogIndex].pReg = szUNKN;
	}
	RegLog[RegLogIndex].dwVal = value;
		...
		if(RegLog[DumpIndex].pReg!=NULL)
		{
			if(RegLog[DumpIndex].count == 1)
				sprintf(buffer,"%s %s > %08.8X      %c\r\n",
						RegLog[DumpIndex].read?"read ":"write",
						RegLog[DumpIndex].pReg,RegLog[DumpIndex].dwVal,
						RegLog[DumpIndex].error?'!':' ');
			else
				sprintf(buffer,"%s %s > %08.8X [%d] %c\r\n",
						RegLog[DumpIndex].read?"read ":"write",
						RegLog[DumpIndex].pReg,RegLog[DumpIndex].dwVal,
						RegLog[DumpIndex].count,RegLog[DumpIndex].error?'!':' ');
...
void MessageBuffer_Sprintf(const char *pFmt,...)
{
	...
	for(idx=0;idx<len;idx++)
	{
		...
		if(MessageBufferCurOffset==MESSAGE_BUFFER_LENGTH)
			MessageBufferCurOffset = MESSAGE_BUFFER_START_STRING_LENGTH;
	}
...
// pMemBase is pointer to HW32
#define REG56301_HCTR      (0x0010/sizeof(HW32))    // HI32 control reg
...
#define REG56301_HSTR       (0x0014/sizeof(HW32))    // HI32 status reg
...
#define REG56301_HCVR       (0x0018/sizeof(HW32))   // HI32 command vector reg

#define REG56301_HTXR       (0x001C/sizeof(HW32))	// Host Transmit data reg (FIFO)
#define REG56301_HRXS       (0x001C/sizeof(HW32))	// Host Receive data reg (FIFO)
...
typedef struct
{
    HW16    wSize;
    HW16    wStreamIndex;
    HW32    wData[DPI_DATA_SIZE];
}
DPI_DATA_MESSAGE;

// HW32 dwLast_HCVR=CVR_RESET|CVR_INT;

/**************************** LOCAL PROTOTYPES **************************/
static HW32		Dpi_SafeReadHSTR(HPI_56301_INFO_OBJ * pio);
static HW32		Dpi_SafeReadHCVR(HPI_56301_INFO_OBJ * pio);
static void		Dpi_SafeWriteHCVR(HPI_56301_INFO_OBJ * pio, HW32 dwHCVR);

static short    Dpi_Command(HPI_56301_INFO_OBJ * pio, HW32 Cmd);
...
static short    DpiData_Read16(HPI_56301_INFO_OBJ * pio, HW16 HUGE *pwData);
static short    DpiData_Read32(HPI_56301_INFO_OBJ * pio, HW32 HUGE *pdwData);
static short    DpiData_Write32(HPI_56301_INFO_OBJ * pio, HW32 HUGE *pdwData);
static short 	DpiData_WriteBlock16(HPI_56301_INFO_OBJ * pio, HW16 HUGE *pwData, HW32 dwLength);
...
#endif
static short 	DpiData_Write24(HPI_56301_INFO_OBJ * pio, HW32 HUGE *pdwData);
...
#if 0
short Hpi56301_SelfTest( HW32 pMemBase)
{
...
/************************************************************************/
//short Hpi56301_BootLoadDsp( HW32 pMemBase, HW32 HFAR * apaDspCodeArrays[] ) old

short Hpi56301_BootLoadDsp( HPI_ADAPTER_OBJ * pao, HPI_56301_INFO_OBJ * pio, HW32 *pdwOsErrorCode )
{
	 __iomem HW32 * pMemBase=pio->pMemBase;
    DSP_CODE DspCode;
    short nError=0;
...
#define DOTRATE 102
	 ...
	 {
		  dwData=(HW32)adwDspCode_Boot4000a[(HW16)i];
	 ...
	 while( 1 )
	 {
        ...
        if ((nError = HpiDspCode_ReadWord( &DspCode,&dwDspCodeAddr))!=0)
            goto exit;
        if ((nError = HpiDspCode_ReadWord( &DspCode,&dwDspCodeType))!=0)
            goto exit;
        ...
        for(i=0; i<dwDspCodeLength; i++)
        {
            ...
            if ((nError=HpiDspCode_ReadWord(&DspCode,&dwData))!=0)
                goto exit;
            if (DpiData_Write24( pio, &dwData))
            {
...
/************************************************************************/
static HW16 Hpi56301_Resync(HPI_56301_INFO_OBJ * pio)
{
    HW16 wError;
    //?	 HW32 dwData;
    HW32 dwTimeout;
    ...
    dwTimeout=TIMEOUT;
    while (dwTimeout && (HPIOS_MEMREAD32(pio->pMemBase+REG56301_HSTR) & HSTR_HRRQ) )
    {
                    ...
                    short wDataType )
{
	...
	if(!(Dpi_SafeReadHSTR(pio)&1))
	{
		// XMIT FIFO NOT EMPTY
		DumpRegisterLog();
	    HPI_DEBUG_LOG0(ERROR,"Xmit FIFO not empty in Hpi56301_Send()\n");
		Hpi56301_Resync(pio);
                   ...
                   HW32 dwMaxLengthInWords )
{
    ...
    *pdwLengthInWords=dwLength1+dwLength2;		// AGE Feb 19 98
    ...
    {
        HPI_DEBUG_LOG1(DEBUG,"Unexpected data wrap for response, got %u\n", dwAfterWrap);
...
void Hpi56301_Message( HPI_56301_INFO_OBJ * pio, HPI_MESSAGE *phm, HPI_RESPONSE *phr)
{
    ...
    nError=Hpi56301_Get(pio,
                        ...
                        &dwSize,
                        CMD_GET_RESP,
						sizeof(HPI_RESPONSE)/sizeof(HW16) );
    if(nError)
    {
        phr->wError=nError;
        return;
    ...
    if (phm->wFunction==HPI_OSTREAM_WRITE)
    {
		...
		nError=Hpi56301_Send(pio,
                             (HW16 HUGE *)phm->u.d.u.Data.pbData,
            ...
            && (phm->u.cx.wAttribute == HPI_AES18_MESSAGE))
    {
        ...
        if (phm->wFunction==HPI_CONTROL_GET_STATE)
        {
            ...
            nError=Hpi56301_Get(pio,
                                ...
                                &dwSize,
                                CMD_GET_DATA,
								phm->u.cx.u.aes18rx_get_message.wQueueSize/sizeof(HW16));
            if(nError)
            {
                phr->wError=nError;
                return;
...
static HW32	Dpi_SafeReadHSTR(HPI_56301_INFO_OBJ * pio)
{
	...
	do
	{
		...
		if(dwHSTR&CVR_NMI)
		{
			...
			HPI_DEBUG_LOG1(DEBUG,"Dpi_SafeReadHSTR() bad read.  Value:0x%08x\r\n",dwHSTR);
		}
		else
			break;
	}
	...
	{
		HPI_DEBUG_LOG1(ERROR,"Dpi_SafeReadHSTR() failed.  Value:0x%08x\r\n",dwHSTR);
	}
    return dwHSTR;
}
...
static HW32	Dpi_SafeReadHCVR(HPI_56301_INFO_OBJ * pio)
{
	...
	do
	{
		...
		if((dwHCVR|CVR_INT)!=pio->dwHCVR)
		{
			...
			HPI_DEBUG_LOG1(DEBUG,"Dpi_SafeReadHCVR() bad read.  Value:0x%08x\r\n",dwHCVR);
		}
		else
			break;
	}
	...
	{
		HPI_DEBUG_LOG1(ERROR,"Dpi_SafeReadHCVR() failed.  Value:0x%08x\r\n",dwHCVR);
	}
	return dwHCVR;
}
...
//	HW32 dwHSTR;
    ...
    if (!dwTimeout)
    {
        HPI_DEBUG_LOG2(DEBUG,"Dpi_WaitFlags()  Expecting %d, got %d\r\n",Flagval, wFlags);
...
static HW32 Dpi_Command_Handshake(HPI_56301_INFO_OBJ * pio,HW32 Cmd)
{
	HW32 dwTimeout;
	HW32 dwHCVR;
//	HW32 dwHCTR;
...
/************************************************************************/
static HW32 DpiData_ReadHandshake(HPI_56301_INFO_OBJ * pio)
{
	HW32 dwTimeout;
	HW32 dwHSTR;
	dwTimeout = TIMEOUT;
...
/************************************************************************/
static HW32 DpiData_WriteHandshake(HPI_56301_INFO_OBJ * pio, HW32 dwBitMask)
{
	HW32 dwTimeout;
	HW32 dwHSTR;
...
// Writing the FIFO, protected against FIFO full
// Returns error if FIFO stays full
static short DpiData_Write16(HPI_56301_INFO_OBJ * pio, HW16 HUGE *pwData)
{
	HW32 wData;
...
short DpiData_ReadBlock32(HW32 dwBase, HW32 *pdwData, HW32 dwLength)
{
    HW32 dwTimeout = TIMEOUT;
    HW32 dwAddrFifoStatus = dwBase+REG56301_HSTR;
    HW32 dwAddrFifoRead = dwBase + REG56301_HRXS;
    HW32 i;
...
short DpiData_WriteBlock32(HW32 dwBase, HW32 HUGE *pdwData, HW32 dwLength)
{
    HW32 dwTimeout = TIMEOUT;
    HW32 dwAddrFifoStatus = dwBase+REG56301_HSTR;
    HW32 dwAddrFifoWrite = dwBase + REG56301_HTXR;
    HW32 dwD0, dwD1, dwD2, dwD3, dwD4, dwD5;
    HW32 dwLeft;
