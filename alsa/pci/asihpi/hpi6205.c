...
/* write it */
		...
		HW_Message(pao, phm, phr);
/* update status information that the DSP would typically update */
/* (and will update next time the DSP */
...
/* buffer land in internel BBM buffer */
...
/* errors fatal to this interface module */
	if ((phr->wError >= HPI6205_ERROR_BASE) && (phr->wError <= 4000))
		HPI_DEBUG_LOG(ERROR,
			"HPI Response - error# %d, adapter# %d, function# %d\n",
