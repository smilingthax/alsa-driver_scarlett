...
#define HPI_Message(m,r) HPI_MessageEx((m),(r),(void *)NULL)
#endif

#define HPI_UNUSED(param) (void)param

void HPI_InitMessage( HPI_MESSAGE *phm, HW16 wObject, HW16 wFunction );
HPI_HANDLE HPI_API HPI_IndexesToHandle3(
    const char cObject, ///< HPI_OBJ_* the type of object
...
HPI_HANDLE HPI_API HPI_IndexesToHandle(
    const char cObject, ///< HPI_OBJ_* the type of object
    const HW16 wAdapterIndex, ///< The Adapter index
    const HW16 wObjectIndex ///< The stream or control index, if used
);

void HPI_HandleToIndexes3( const HPI_HANDLE dwHandle, HW16 *pwIndex1, HW16 *pwIndex2 , HW16 *pwIndex0 );
...
/** HPI Subsystem free.
  ...
  */
void HPI_SubSysFree(
    HPI_HSUBSYS *phSubSys	///< Pointer to HPI subsystem handle.
)
{
    HPI_MESSAGE     hm;
	 HPI_RESPONSE    hr;
...
/** Find all adapters that the HPI subsystem knows about.
  *
  * The type and adapter number of each adapter is returned in an array of HW16 pointed to by 
...
HW16 HPI_SubSysFindAdapters(
    HPI_HSUBSYS *phSubSys,	///< Pointer to HPI subsystem handle.
    HW16 *pwNumAdapters,	///< Returned number of initialised adapters in the audio sub-system.
    HW16 awAdapterList[],	///< Array of adapter types (see description).
    HW16 wListLength		///< Size (in elements) of *pawAdapterList array.
)
{
    HPI_MESSAGE hm;
    HPI_RESPONSE hr;
...
HW16 HPI_SubSysCreateAdapter(
    ...
    HW16         *pwAdapterIndex	///< Returned index of the adapter that was just created.
)
{
    HPI_MESSAGE hm;
    HPI_RESPONSE hr;
...
HW16 HPI_SubSysDeleteAdapter(
    HPI_HSUBSYS *phSubSys,			///< Pointer to HPI subsystem handle.
    HW16    wAdapterIndex			///< Index of the adapter to delete.
)
{
    HPI_MESSAGE hm;
    HPI_RESPONSE hr;
...
HW16 HPI_AdapterOpen(
    HPI_HSUBSYS *phSubSys,	///< Pointer to HPI subsystem handle.
    HW16 wAdapterIndex		///< Index of adapter to be opened. Ranges from 0 to 15 and corresponds to the Adapter Index set on the adapter hardware.
)
{
    HPI_MESSAGE hm;
    HPI_RESPONSE hr;
...
HW16 HPI_AdapterClose(
    HPI_HSUBSYS *phSubSys,	///< Pointer to HPI subsystem handle.
    HW16 wAdapterIndex		///< Index of adapter to be closed. Ranges from 0 to 15 and corresponds to the Adapter Index set on the adapter hardware.
)
{
    HPI_MESSAGE hm;
    HPI_RESPONSE hr;
...
HW16 HPI_AdapterFindObject(
    const HPI_HSUBSYS *phSubSys, ///< Pointer to HPI subsystem handle.
    HW16  wAdapterIndex,  ///< Index of adapter to search.
    HW16  wObjectType,    ///< Type of object HPI_OBJ_*.
    HW16  wObjectIndex,	  ///< Index of object
    HW16  *pDspIndex      ///< Return the index of the DSP containing the object.
)
{
    HPI_MESSAGE hm;
    HPI_RESPONSE hr;
...
HW16 HPI_AdapterSetMode(
	 HPI_HSUBSYS *phSubSys,  ///< Pointer to HPI subsystem handle.
	 HW16 wAdapterIndex,     ///< Index of adapter to set mode on.
	 HW32 dwAdapterMode      ///< The adapter mode. See list of modes in the function description.
)
{
...
HW16 HPI_AdapterSetModeEx(
	 HPI_HSUBSYS *phSubSys,  ///< Pointer to HPI subsystem handle.
	 HW16 wAdapterIndex,     ///< Index of adapter to set mode on.
	 HW32 dwAdapterMode,     ///< The adapter mode. See list of modes in the function HPI_AdapterSetMode().
	 HW16 wQueryOrSet
)
{
	 HPI_MESSAGE hm;
...
HW16 HPI_AdapterGetMode(
    HPI_HSUBSYS *phSubSys,  ///< Pointer to HPI subsystem handle.
    HW16 wAdapterIndex,     ///< Index of adapter to get mode from.
    HW32 *pdwAdapterMode    ///< The returned adapter mode. Will be one of HPI_ADAPTER_MODE_XXXX.
)
{
    HPI_MESSAGE hm;
    HPI_RESPONSE hr;
...
HW16 HPI_AdapterGetInfo(
    HPI_HSUBSYS *phSubSys,  ///< Pointer to HPI subsystem handle.
    HW16 wAdapterIndex,     ///< Index of adapter to read mode from.
    HW16 *pwNumOutStreams,  ///< Number of output streams (play) on the adapter.
    HW16 *pwNumInStreams,   ///< Number of input streams (record) on the adapter.
    //HW8   szAdapterName[],
    //HW16 wStringLen
    HW16 *pwVersion,
    HW32 *pdwSerialNumber,  ///< Adapter serial number.  Starts at 1 and goes up.
    HW16 *pwAdapterType     ///< Adapter ID code, defined in HPI.H.  Examples are HPI_ADAPTER_ASI6114 (0x6114).
)
{
    HPI_MESSAGE hm;
    HPI_RESPONSE hr;
...
HW16 HPI_AdapterGetAssert(
    HPI_HSUBSYS *phSubSys,    ///< Pointer to HPI subsystem handle.
    HW16    wAdapterIndex,    ///< Adpater index to read assert from.
    HW16    *wAssertPresent,  ///< Set to 1 if an assert was returned, otherwise it returns 0.
	...
	HW16    *pwLineNumber     ///< The line number that caused the assert.
)
{
    HPI_MESSAGE hm;
    HPI_RESPONSE hr;
...
HW16 HPI_AdapterGetAssertEx(
	 HPI_HSUBSYS *phSubSys, ///< HPI subsystem handle.
...
HW16 HPI_AdapterTestAssert(
    HPI_HSUBSYS *phSubSys,  ///< Pointer to HPI subsystem handle.
    HW16    wAdapterIndex,  ///< Index of adapter to generate the test assert.
    HW16    wAssertId       ///< An assert id number that is returned as the line number in HPI_AdapterGetAssert().
)
{
    HPI_MESSAGE hm;
    HPI_RESPONSE hr;
...
and should be set to 0.
	\return HPI_ERROR_*
*/
HW16 HPI_AdapterSetProperty(
	HPI_HSUBSYS 	*phSubSys, ///< HPI subsystem handle.
	HW16			wAdapterIndex,   ///< Adapter index.
	HW16            wProperty,      ///< Adapter property to set. One of HPI_ADAPTER_PROPERTY_*.
	HW16            wParameter1,     ///< Adapter property parameter 1.
	HW16            wParameter2      ///< Adapter property parameter 2.
	)
{
    HPI_MESSAGE hm;
    HPI_RESPONSE hr;
...
property and should be set to 0.
	\return HPI_ERROR_*
*/
HW16 HPI_AdapterGetProperty(
	HPI_HSUBSYS 	*phSubSys, ///< HPI subsystem handle.
	HW16			wAdapterIndex,   ///< Adapter index.
	HW16            wProperty,       ///< Adapter property to set. One of HPI_ADAPTER_PROPERTY_*.
	HW16            *pwParameter1,    ///< Returned adapter property parameter 1.
	HW16            *pwParameter2     ///< Returned adapter property parameter 2.
	)
{
    HPI_MESSAGE hm;
    HPI_RESPONSE hr;
...
the settings for a particular propery can also be established.
	\return HPI_ERROR_*
*/
HW16 HPI_AdapterEnumerateProperty(
	HPI_HSUBSYS 	*phSubSys,  ///< HPI subsystem handle.
	HW16			wAdapterIndex,    ///< Adapter index.
	HW16            wIndex,           ///< Adapter property # to return.
	HW16			wWhatToEnumerate, ///< Either HPI_ADAPTER_PROPERTY_ENUMERATE_PROPERTIES or HPI_ADAPTER_PROPERTY_ENUMERATE_SETTINGS
	HW16            wPropertyIndex,   ///< Property index.
	HW32            *pdwSetting       ///< Returned adapter property, or property setting, depending on wWhatToEnumerate.
	)
{
	return 0;
}
...
)
{
    ...
    if(hr.wError==0)
        *phOutStream = HPI_IndexesToHandle( HPI_OBJ_OSTREAM, wAdapterIndex, wOutStreamIndex );
    else
        *phOutStream = 0;
    return(hr.wError);
}
...
)
{
    ...
    HPI_HANDLETOINDEXES( hOutStream, &hm.wAdapterIndex, &hm.u.d.wStreamIndex );
    HPI_Message( &hm, &hr );
    ...
    HW32            *pdwAuxiliaryDataToPlay)
{
    ...
    if(pdwAuxiliaryDataToPlay)
        *pdwAuxiliaryDataToPlay = hr.u.d.u.stream_info.dwAuxiliaryDataAvailable;
    return(hr.wError);
}
...
*/
HW16 HPI_OutStreamWriteBuf(
	HPI_HSUBSYS *phSubSys, ///< subystem handle
	HPI_HOSTREAM hOutStream, ///< stream handle
	HW8 *        pbData, ///< Pointer to buffer containing data
	HW32         dwBytesToWrite, ///< Number of bytes to write, must be <= space available
    ...
    HW32	*pdwFramesAvailable)
{
	 ...
	 if(hr.wError==0)
    {
        if(pdwFramesAvailable)
            *pdwFramesAvailable = hr.u.d.u.stream_info.dwDataAvailable/sizeof(HPI_ANC_FRAME);
    }
    return(hr.wError);
}
	 ...
	 HW32	dwNumberOfAncillaryFramesToRead)
{
    ...
    HPI_HANDLETOINDEXES( hOutStream, &hm.wAdapterIndex, &hm.u.d.wStreamIndex );
    hm.u.d.u.Data.pbData = (HW8 *)pAncFrameBuffer;
				...
				HPI_HSTREAM hStreamHandle )
{
    ...
    hr.wError=0;
    HPI_HANDLETOINDEXES( hOutStreamHandle, &hm.wAdapterIndex, &hm.u.d.wStreamIndex );
	cObjType = HPI_HandleObject( hStreamHandle );
	switch(cObjType){
	case HPI_OBJ_OSTREAM:
		hm.u.d.u.Stream.wObjectType = HPI_OBJ_OSTREAM;
		HPI_HANDLETOINDEXES( hStreamHandle, &wAdapter, &hm.u.d.u.Stream.wStreamIndex );
		break;
	case HPI_OBJ_ISTREAM:
		hm.u.d.u.Stream.wObjectType = HPI_OBJ_ISTREAM;
	    HPI_HANDLETOINDEXES3( hStreamHandle, &wAdapter, &hm.u.d.u.Stream.wStreamIndex , &wDspIndex);
		if (wDspIndex != 0)
    ...
    if(hr.wError==0)
	 {
        ...
        // and stream index
        if(hr.wError==0)
            *phInStream = HPI_IndexesToHandle3( HPI_OBJ_ISTREAM, wAdapterIndex, wInStreamIndex , wDspIndex);
        else
            *phInStream = 0;
    }
    else
...
)
{
    ...
    HPI_HANDLETOINDEXES3( hInStream, &hm.wAdapterIndex, &hm.u.d.wStreamIndex , &hm.wDspIndex);
    HPI_Message( &hm, &hr );
...
*/
HW16 HPI_InStreamReadBuf(
    HPI_HSUBSYS *phSubSys,  ///< subsystem handle
    HPI_HISTREAM hInStream, ///< InStream handle
    HW8 * pbData,       ///< Pointer to buffer for read data
    HW32 dwBytesToRead ///< Number of bytes to read, must be <= number available
)
{
    HPI_MESSAGE hm;
    HPI_RESPONSE hr;
...
)
{
    ...
    if(pdwAuxiliaryDataRecorded)
        *pdwAuxiliaryDataRecorded = hr.u.d.u.stream_info.dwAuxiliaryDataAvailable;
    return(hr.wError);
}
    ...
    HW32	*pdwFrameSpace)
{
    ...
    HPI_Message( &hm, &hr );
    if(pdwFrameSpace)
        *pdwFrameSpace = (hr.u.d.u.stream_info.dwBufferSize-hr.u.d.u.stream_info.dwDataAvailable)/sizeof(HPI_ANC_FRAME);
    return(hr.wError);
}
				...
				HPI_HSTREAM hStreamHandle )
{
    ...
    hr.wError=0;
    HPI_HANDLETOINDEXES3( hInStreamHandle, &hm.wAdapterIndex, &hm.u.d.wStreamIndex , &hm.wDspIndex);
	cObjType = HPI_HandleObject( hStreamHandle );
	switch(cObjType){
	case HPI_OBJ_OSTREAM:
		hm.u.d.u.Stream.wObjectType = HPI_OBJ_OSTREAM;
...
)
{
    ...
    if(hr.wError==0)
		*phMixerHandle = HPI_IndexesToHandle( HPI_OBJ_MIXER, wAdapterIndex, 0 );
	else
		*phMixerHandle = 0;
    return(hr.wError);
}
...
)
{
    ...
    if(hr.wError==0)
        *phControlHandle = HPI_IndexesToHandle( HPI_OBJ_CONTROL, hm.wAdapterIndex, hr.u.m.wControlIndex );
    else
        *phControlHandle = 0;
    return(hr.wError);
}
...
)
{
	 ...
	 *pwControlType =hr.u.m.wControlIndex; /* Actually Type */
	 ...
	 {
		 if(hr.wError==0)
			  *phControlHandle = HPI_IndexesToHandle(HPI_OBJ_CONTROL, hm.wAdapterIndex, wControlIndex );
		 else
			  *phControlHandle = 0;
	 }
	 return(hr.wError);
...
static HW16 HPI_ControlExParamSet(
	 const HPI_HSUBSYS     *phSubSys,
	 const HPI_HCONTROL    hControlHandle,
	 const HW16            wAttrib,
	 const HW32            dwParam1,
...
static HW16 HPI_ControlExParamGet(
	 const HPI_HSUBSYS     *phSubSys,
	 const HPI_HCONTROL    hControlHandle,
	 const HW16            wAttrib,
	 HW32                  dwParam1,
...
{
	return HPI_ControlParamGet(phSubSys,hControlHandle,wAttrib,0,0,pdwParam1,pdwParam2);
}
...
{
	return HPI_ControlParamGet(phSubSys,hControlHandle,wAttrib,0,0,pdwParam1,0L);
}
#endif
...
{
    return HPI_ControlParamSet(phSubSys, hControlHandle, HPI_AESEBU_SOURCE, wSource, 0);
}
...
)
{
	...
	wErr = HPI_ControlParam1Get(phSubSys, hControlHandle, HPI_AESEBU_SOURCE, &dwParam);
	if(!wErr && pwSource)
...
{
    return HPI_ControlParam1Get(phSubSys, hControlHandle, HPI_AESEBU_SAMPLERATE,pdwSampleRate);
}
...
)
{
	...
	wError = HPI_ControlParam1Get(phSubSys, hControlHandle, HPI_AESEBU_ERRORSTATUS,&dwErrorData);
    if(pwErrorData)
...
{
    return HPI_ControlParamSet(phSubSys, hControlHandle, HPI_AESEBU_SAMPLERATE, dwSampleRate, 0);
}
...
{
    return HPI_ControlParamSet(phSubSys, hControlHandle, HPI_AESEBU_USERDATA, wIndex, wData);
}
...
{
    return HPI_ControlParamSet(phSubSys, hControlHandle, HPI_AESEBU_CHANNELSTATUS, wIndex, wData);
}
...
{
    return HPI_ControlParamSet(phSubSys, hControlHandle, HPI_AESEBU_CLOCKSOURCE, wClockSource, 0);
}
...
)
{
	...
	wErr = HPI_ControlParam1Get(phSubSys, hControlHandle, HPI_AESEBU_CLOCKSOURCE, &dwParam);
...
)
{
    ...
    return HPI_ControlParamSet(phSubSys, hControlHandle, HPI_AESEBU_SOURCE, wOutputFormat, 0);
}
...
{
    return HPI_ControlParamSet(phSubSys, hControlHandle, HPI_BITSTREAM_CLOCK_EDGE, wEdgeType,0);
}
...
{
    return HPI_ControlParamSet(phSubSys, hControlHandle, HPI_BITSTREAM_DATA_POLARITY, wPolarity,0);
}
...
{
    return HPI_ControlParamSet(phSubSys, hControlHandle, HPI_MULTIPLEXER_SOURCE, wMode, 0);
}
...
{
    HW32 dwMode=0;
	HW16 wError=HPI_ControlParam1Get(phSubSys, hControlHandle, HPI_MULTIPLEXER_SOURCE, &dwMode);
    if(wMode)
        *wMode = (HW16)dwMode;
    return(wError);
}
...
{
	return HPI_ControlExParamSet(phSubSys, hControlHandle, HPI_COBRANET_MODE, dwMode, dwSetOrQuery);
}
				...
				)
{
	HW16 nError=0;
	HW32 dwMode=0;
	nError = HPI_ControlExParam1Get(phSubSys, hControlHandle, HPI_COBRANET_MODE, &dwMode);
	...
	)
{
	...
	nError=HPI_ControlParam2Get(phSubSys,hControlHandle,HPI_METER_RMS_BALLISTICS,
					&dwAttack, &dwDecay);
	...
	)
{
	...
	nError=HPI_ControlParam2Get(phSubSys,hControlHandle,HPI_METER_PEAK_BALLISTICS,
					&dwAttack, &dwDecay);
...
{
	return HPI_ControlParamSet(phSubSys, hControlHandle, HPI_MICROPHONE_PHANTOM_POWER, (HW32)wOnOff, 0);
}
				...
				)
{
	HW16 nError=0;
	HW32 dwOnOff=0;
	nError = HPI_ControlParam1Get(phSubSys, hControlHandle, HPI_MICROPHONE_PHANTOM_POWER, &dwOnOff);
...
{
    return HPI_ControlParamSet(phSubSys, hControlHandle, HPI_MULTIPLEXER_SOURCE, wSourceNodeType, wSourceNodeIndex);
}
...
{
    HW32 dwNode,dwIndex;
	 HW16 wError=HPI_ControlParam2Get(phSubSys, hControlHandle, HPI_MULTIPLEXER_SOURCE, &dwNode, &dwIndex);
    if(wSourceNodeType)
...
{
    return HPI_ControlParamSet(phSubSys, hControlHandle, HPI_ONOFFSWITCH_STATE, wState, 0);
}
...
{
    HW32 dwState=0;
	HW16 wError=HPI_ControlParam1Get(phSubSys, hControlHandle, HPI_ONOFFSWITCH_STATE, &dwState);
    if(wState)
        *wState = (HW16)dwState;
    return(wError);
}
		...
		)
{
	...
	nError = HPI_ControlParam2Get(
		phSubSys, hControlHandle, HPI_EQUALIZER_NUM_FILTERS,
		&dwOO, &dwNOB);
	 if(pwNumberOfBands)
...
{
    return HPI_ControlParamSet(phSubSys, hControlHandle, HPI_SAMPLECLOCK_SOURCE, wSource,0);
}
...
{
    return HPI_ControlParamSet(phSubSys, hControlHandle, HPI_SAMPLECLOCK_SOURCE_INDEX, wSourceIndex,0);
}
...
)
{
	HW16 wError=0;
	HW32 dwSource=0;
	wError = HPI_ControlParam1Get(phSubSys, hControlHandle, HPI_SAMPLECLOCK_SOURCE, &dwSource);
	if(!wError)
		if(pwSource)
...
)
{
	...
	wError = HPI_ControlParam1Get(phSubSys, hControlHandle, HPI_SAMPLECLOCK_SOURCE_INDEX, &dwSourceIndex);
	if(!wError)
		if(pwSourceIndex)
...
{
	return HPI_ControlParamSet(phSubSys, hControlHandle, HPI_SAMPLECLOCK_SAMPLERATE, dwSampleRate,0);
}
...
)
{
	HW16 wError=0;
	HW32 dwSampleRate=0;
	wError = HPI_ControlParam1Get(phSubSys, hControlHandle, HPI_SAMPLECLOCK_SAMPLERATE, &dwSampleRate);
	if(!wError)
		if(pdwSampleRate)
...
HW16 HPI_ToneDetector_GetFrequency(
                HPI_HSUBSYS     *phSubSys,
...
{
	return HPI_ControlParamGet(phSubSys, hControl, HPI_TONEDETECTOR_FREQUENCY, nIndex,0, dwFrequency, NULL);
}
///\}
...
{
    return HPI_ControlParamSet(phSubSys, hControlHandle, HPI_TUNER_BAND, wBand, 0);
}
...
{
	 return HPI_ControlParamSet(phSubSys, hControlHandle, HPI_TUNER_GAIN, nGain, 0);
}
...
{
    return HPI_ControlParamSet(phSubSys, hControlHandle, HPI_TUNER_FREQ, wFreqInkHz, 0);
}
...
)
{
	...
	nError = HPI_ControlParam1Get(phSubSys, hControlHandle, HPI_TUNER_BAND, &dwBand);
	 if(pwBand)
		  *pwBand = (HW16)dwBand;
	 return nError;
}
...
)
{
	...
	nError = HPI_ControlParam1Get(phSubSys, hControlHandle, HPI_TUNER_GAIN, &dwGain);
	 if(pnGain)
		  *pnGain = (HW16)dwGain;
	 return nError;
}
...
{
    return HPI_ControlParam1Get(phSubSys, hControlHandle, HPI_TUNER_FREQ, pwFreqInkHz);
}
...
)
{
    ...
    HW16    nError=0;
	 ...
	 nError = HPI_ControlParam1Get(phSubSys, hControlHandle, HPI_TUNER_VIDEO_STATUS, &dwStatus);
    if(pwStatus)
...
)
{
	...
	nError = HPI_ControlParam1Get(phSubSys, hControlHandle, HPI_TUNER_STATUS, &dwStatus);
    if(pwStatus)
    {
		if(!nError)
		{
...
{
	 return HPI_ControlParamSet(phSubSys, hControlHandle, HPI_TUNER_MODE, nMode, nValue);
}
...
{
	 return HPI_ControlParamGet(phSubSys, hControlHandle, HPI_TUNER_MODE, nMode,0, pnValue,NULL);
}
/**\}*/
/////////////////////////////////////////////////////////////////////////
...
void ThreadAsync()
{
	...
	while(!ExitSignalled)
	{
		wError = HW16 HPI_AsyncEventWait( pSS, hAsync,
			MAX_EVENTS,
			&e[0],
			&wEvents);
...
\return 0==success or HPI_ERROR_*
*/
HW16 HPI_AsyncEventOpen(
		  HPI_HSUBSYS     *phSubSys, ///< Subsystem handle.
		  HW16            wAdapterIndex, ///< The adapter index to open the Async object.
...
\return 0==success or HPI_ERROR_*
*/
HW16 HPI_AsyncEventWait(
		  HPI_HSUBSYS   *phSubSys, ///< Subsystem handle.
		  HPI_HASYNC    hAsync, ///< Handle of an Async object.
		  HW16			wMaximumEvents, ///< Maximum number of events matches size of array passed in pEvents.
		  HPI_ASYNC_EVENT *pEvents, ///< Events are returned here.
		  HW16			*pwNumberReturned ///< Number events returned.
		  )
{
    return(0);
}
...
\return 0==success or HPI_ERROR_*
*/
HW16 HPI_AsyncEventGetCount(
		  HPI_HSUBSYS   *phSubSys, ///< Subsystem handle.
		  HPI_HASYNC    hAsync, ///< Handle of an Async object.
		  HW16			*pwCount /// Returned number of events waiting.
		  )
{
    HPI_MESSAGE hm;
    HPI_RESPONSE hr;
...
)
{
    ...
    *pwMaxProfiles = hr.u.p.u.o.wMaxProfiles;
    if(hr.wError==0)
        *phProfile = HPI_IndexesToHandle(HPI_OBJ_PROFILE, wAdapterIndex, wProfileIndex );
    else
        *phProfile = 0;
    return(hr.wError);
}
...
)
{
    ...
    pAes18MsgRxGetMessage->dwpbMessage = (HW32)pbMessage;
...
)
{
    ...
    */
    ...
    pAes18SendMsg->dwpbMessage = (HW32)pbMessage;
...
HW32 gadwHpiSpecificError[4]={0,0,0,0};
...
void HPI_GetErrorText( HW16 wError, char *pszErrorText)
{
    ...
    case HPI_ERROR_DSP_FILE_NO_HEADER:	//212
        strcat( pszErrorText, "First DSP code section header not found in DSP file");
        break;
    case HPI_ERROR_DSP_FILE_READ_ERROR:	  //213
        strcat( pszErrorText, "File read operation on DSP code file failed");
        break;
    ...
    case HPI_ERROR_DSP_FILE_OTHER_ERROR:  //215
        strcat( pszErrorText, "Other OS specific error opening DSP file");
        break;
    ...
    case HPI_ERROR_BAD_CHECKSUM:      //221
        strcat( pszErrorText, "Flash - could not determine a valid checksum");
        break;
    case HPI_ERROR_BAD_SEQUENCE:      //222
        strcat( pszErrorText, "Flash - bad packet sequence number during flash programming");
        break;
    ...
    case HPI_ERROR_FLASH_START:      //227
        strcat( pszErrorText, "Flash - command to start programming sequence failed");
        break;
    ...
    case HPI_ERROR_TOO_MANY_CAPABILITY_CHANGE_ATTEMPTS:	// 313
        strcat( pszErrorText, "There have been too many attempts to set the adapter's "
    ...
    case HPI_ERROR_NO_INTERADAPTER_GROUPS:	// 314
        strcat( pszErrorText, "Streams on different adapters cannot be grouped.");
        break;
    case HPI_ERROR_NO_INTERDSP_GROUPS:		// 315
        strcat( pszErrorText, "Streams on different DSPs cannot be grouped.");
        break;
...
)
{
    ...
    case HPI_FORMAT_PCM16_SIGNED:
	case HPI_FORMAT_PCM16_BIGENDIAN:
		Format.dwBitRate = (HW32)wChannels * dwSampleRate * 2;
		break;
	case HPI_FORMAT_PCM32_SIGNED:
	case HPI_FORMAT_PCM32_FLOAT:
		Format.dwBitRate = (HW32)wChannels * dwSampleRate * 4;
        break;
	case HPI_FORMAT_PCM8_UNSIGNED:
        Format.dwBitRate = (HW32)wChannels * dwSampleRate;
        break;
    default:
        Format.dwBitRate=0;
	 }
...
/** allow conversion from handle to HW32 */
typedef union {
  struct sHANDLE h;
  HW32 w;
} tHANDLE;
...
*/
HPI_HANDLE HPI_IndexesToHandle3(
    const char cObject, ///< HPI_OBJ_* - the type code of object
    const HW16 wAdapterIndex, ///< The Adapter index
    const HW16 wObjectIndex, ///< The stream or control index, if used
    const HW16 wDspIndex  ///< The index of the DSP which implements the object, usually 0
)
{
  tHANDLE handle;
...
void HPI_HandleToIndexes3(
    const HPI_HANDLE dwHandle, ///< The handle to decode
