...
static inline void HW_Message( HPI_ADAPTER_OBJ *pao, HPI_MESSAGE *phm, HPI_RESPONSE *phr);
...
static void MixerMessage( HPI_ADAPTER_OBJ *pao, HPI_MESSAGE *phm, HPI_RESPONSE *phr ) {
...
static short CreateAdapterObj( HPI_ADAPTER_OBJ *pao, HW32 *pdwOsErrorCode )
{
    ...
    pao->wOpen = 0;
        ...
        if(HpiPci_FindDevice( pao->Pci, nIndex, HPI_PCI_VENDOR_ID_MOTOROLA, HPI_ADAPTER_DSP56301))
        {
        ...
        HpiPci_WriteConfig( &pao->Pci, HPIPCI_CCMR, HPIPCI_CCMR_MSE | HPIPCI_CCMR_PERR | HPIPCI_CCMR_BM );

    // Is it really a 301 chip?
    if(Hpi56301_CheckAdapterPresent(&((HPI_HW_OBJ *)pao->priv)->hpi56301info))
    {
    ...
    if (0 != (nBootError = Hpi56301_BootLoadDsp(pao,&((HPI_HW_OBJ *)pao->priv)->hpi56301info,pdwOsErrorCode)))
    {
...
/* Hpi56301_SelfTest doesnt do anything */
...

    {
        ...
        HPIOS_DEBUG_STRING("CreateAdapterObj - Send ADAPTER_GET_INFO\n" );
        ...
        Hpi56301_Message(&((HPI_HW_OBJ *)pao->priv)->hpi56301info, &hM, &hR);
