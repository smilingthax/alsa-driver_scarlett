...
static inline HPI_ENTRY_POINT HPI_LookupEntryPointFunction( HPI_PCI * PciInfo ) {
    ...
    for ( i = 0; pci_dev_tbl[i].wVendorId != 0; i++ ) {
        if ( pci_dev_tbl[i].wVendorId != PCI_ANY_ID && pci_dev_tbl[i].wVendorId != PciInfo->wVendorId )
            continue;
        if ( pci_dev_tbl[i].wDeviceId != PCI_ANY_ID && pci_dev_tbl[i].wDeviceId != PciInfo->wDeviceId )
            continue;
        if ( pci_dev_tbl[i].wSubSysVendorId != PCI_ANY_ID && pci_dev_tbl[i].wSubSysVendorId != PciInfo->wSubSysVendorId )
            continue;
        if ( pci_dev_tbl[i].wSubSysDeviceId != PCI_ANY_ID && pci_dev_tbl[i].wSubSysDeviceId != PciInfo->wSubSysDeviceId )
            continue;
...
static void HPIMSGX_Reset( HW16 wAdapterIndex );
static HW16 HPIMSGX_Init(HPI_MESSAGE *phm, HPI_RESPONSE *phr );
static void HPIMSGX_Cleanup( HW16 wAdapterIndex, void * hOwner );

static HW16 AdapterPrepare(HW16 wFoundAdapter, HW16 isPnP );
...
static HPI_STREAM_RESPONSE	aRESP_HPI_OSTREAM_OPEN[HPI_MAX_ADAPTERS][HPI_MAX_STREAMS];
static HPI_STREAM_RESPONSE	aRESP_HPI_ISTREAM_OPEN[HPI_MAX_ADAPTERS][HPI_MAX_STREAMS];
...
static void SubSysMessage(HPI_MESSAGE *phm, HPI_RESPONSE *phr, void * hOwner ) {
	...
	if( phm->wAdapterIndex >= HPI_MAX_ADAPTERS && phm->wAdapterIndex != HPIMSGX_ALLADAPTERS ) {
		HPI_InitResponse( phr, HPI_OBJ_ADAPTER, HPI_SUBSYS_OPEN, HPI_ERROR_BAD_ADAPTER_NUMBER );
		return;
	}
	...
	switch(phm->wFunction) {
		...
		case HPI_SUBSYS_DRIVER_LOAD:
			...
			// NOTE: we do it blindly because we don't know which cards are available at this stage
			{
				...
				for (i=0; pci_dev_tbl[i].wVendorId != 0; i++) {
					entry_point_func = (HPI_ENTRY_POINT)pci_dev_tbl[i].drvData;
			...
			HPI_InitResponse( phr, HPI_OBJ_SUBSYSTEM, HPI_SUBSYS_DRIVER_LOAD, 0 );
			return;
...
#endif
			HPI_InitResponse( phr, HPI_OBJ_SUBSYSTEM, HPI_SUBSYS_DRIVER_UNLOAD, 0 );
			return;
		case HPI_SUBSYS_FIND_ADAPTERS:
			if ( !gRESP_HPI_SUBSYS_FIND_ADAPTERS.s.wNumAdapters )
				HPIMSGX_Init(NULL, phr);
			else
				memcpy( phr, &gRESP_HPI_SUBSYS_FIND_ADAPTERS, sizeof(gRESP_HPI_SUBSYS_FIND_ADAPTERS));
			return;
		...
		case HPI_SUBSYS_DELETE_ADAPTER:
			...
			gRESP_HPI_SUBSYS_FIND_ADAPTERS.s.awAdapterList[ phm->wAdapterIndex ] = 0;
...
static void AdapterMessage( HPI_MESSAGE *phm, HPI_RESPONSE *phr, void * hOwner ) {
	...
	if( phm->wAdapterIndex >= HPI_MAX_ADAPTERS ) {
		HPI_InitResponse( phr, HPI_OBJ_ADAPTER, HPI_ADAPTER_OPEN,  HPI_ERROR_BAD_ADAPTER_NUMBER );
		return;
	}
...
static void MixerMessage( HPI_MESSAGE *phm, HPI_RESPONSE *phr ) {
	...
	if( phm->wAdapterIndex >= HPI_MAX_ADAPTERS ) {
		HPI_InitResponse( phr, HPI_OBJ_MIXER, HPI_MIXER_OPEN, HPI_ERROR_BAD_ADAPTER_NUMBER );
		return;
	}
...
static void OStreamMessage( HPI_MESSAGE *phm, HPI_RESPONSE *phr, void * hOwner ) {
	...
	if( phm->wAdapterIndex >= HPI_MAX_ADAPTERS ) {
		HPI_InitResponse( phr, HPI_OBJ_OSTREAM, HPI_OSTREAM_OPEN,  HPI_ERROR_BAD_ADAPTER_NUMBER);
		return;
	}

   	if( phm->u.d.wStreamIndex >= aADAPTER_INFO[phm->wAdapterIndex].wNumOutStreams ) {
		HPI_InitResponse( phr, HPI_OBJ_OSTREAM, HPI_OSTREAM_OPEN,  HPI_ERROR_INVALID_OBJ_INDEX);
		return;
	}
...
static void IStreamMessage( HPI_MESSAGE *phm, HPI_RESPONSE *phr, void * hOwner ) {
	...
	if( phm->wAdapterIndex >= HPI_MAX_ADAPTERS ) {
		HPI_InitResponse( phr, HPI_OBJ_ISTREAM, HPI_ISTREAM_OPEN,  HPI_ERROR_BAD_ADAPTER_NUMBER );
		return;
	}
   	if( phm->u.d.wStreamIndex >= aADAPTER_INFO[phm->wAdapterIndex].wNumInStreams ) {
		HPI_InitResponse( phr, HPI_OBJ_ISTREAM, HPI_ISTREAM_OPEN,  HPI_ERROR_INVALID_OBJ_INDEX);
		return;
	}
...
static void AdapterOpen( HPI_MESSAGE *phm, HPI_RESPONSE *phr ) {
	HPI_DEBUG_LOG0(VERBOSE,"AdapterOpen\n" );
	 memcpy( phr, &aRESP_HPI_ADAPTER_OPEN[ phm->wAdapterIndex ], sizeof( aRESP_HPI_ADAPTER_OPEN[0] ) );
}
...
static void MixerOpen( HPI_MESSAGE *phm, HPI_RESPONSE *phr ) {
	memcpy( phr, &aRESP_HPI_MIXER_OPEN[ phm->wAdapterIndex ], sizeof(aRESP_HPI_MIXER_OPEN[0]) );
}
...
static void InStreamOpen( HPI_MESSAGE *phm, HPI_RESPONSE *phr, void * hOwner ) {
	...
	if( aIStreamUserOpen[phm->wAdapterIndex][phm->u.d.wStreamIndex].nOpenFlag ) {
		phr->wError = HPI_ERROR_OBJ_ALREADY_OPEN;
		goto exit;
	} else if (aRESP_HPI_ISTREAM_OPEN[phm->wAdapterIndex][phm->u.d.wStreamIndex].h.wError) {
		memcpy( phr, &aRESP_HPI_ISTREAM_OPEN[phm->wAdapterIndex][phm->u.d.wStreamIndex],
                        sizeof(aRESP_HPI_ISTREAM_OPEN[0][0]));
		goto exit;
	} else {
		aIStreamUserOpen[phm->wAdapterIndex][phm->u.d.wStreamIndex].nOpenFlag = 1;
		...
		HpiOs_Msgxlock_Lock(&msgxLock, &flags);
		if (hr.wError) {
			aIStreamUserOpen[phm->wAdapterIndex][phm->u.d.wStreamIndex].nOpenFlag = 0;
			phr->wError = hr.wError;
		} else {
			aIStreamUserOpen[phm->wAdapterIndex][phm->u.d.wStreamIndex].nOpenFlag = 1;
			aIStreamUserOpen[phm->wAdapterIndex][phm->u.d.wStreamIndex].hOwner = hOwner;
			memcpy( phr, &aRESP_HPI_ISTREAM_OPEN[phm->wAdapterIndex][phm->u.d.wStreamIndex],
                        sizeof(aRESP_HPI_ISTREAM_OPEN[0][0]));
		}
	}
...
static void InStreamClose( HPI_MESSAGE *phm, HPI_RESPONSE *phr, void * hOwner ) {
	...
	HpiOs_Msgxlock_Lock(&msgxLock, &flags);
	if ( hOwner == aIStreamUserOpen[phm->wAdapterIndex][phm->u.d.wStreamIndex].hOwner ) {
		aIStreamUserOpen[phm->wAdapterIndex][phm->u.d.wStreamIndex].hOwner = NULL;
		HpiOs_Msgxlock_UnLock(&msgxLock, &flags);
		// issue a reset
		memcpy( &hm, phm, sizeof(HPI_MESSAGE));
		hm.wFunction = HPI_ISTREAM_RESET;
		...
		HPI_DEBUG_LOG2(WARNING,"%p trying to close instream owned by %p\n",
			hOwner,
			aIStreamUserOpen[phm->wAdapterIndex][phm->u.d.wStreamIndex].hOwner);
...
static void OutStreamOpen( HPI_MESSAGE *phm, HPI_RESPONSE *phr, void * hOwner ) {
	...
	HpiOs_Msgxlock_Lock(&msgxLock, &flags);
	if( aOStreamUserOpen[phm->wAdapterIndex][phm->u.d.wStreamIndex].nOpenFlag ) {
		phr->wError = HPI_ERROR_OBJ_ALREADY_OPEN;
		goto exit;
	} else if (aRESP_HPI_OSTREAM_OPEN[phm->wAdapterIndex][phm->u.d.wStreamIndex].h.wError) {
		memcpy( phr, &aRESP_HPI_OSTREAM_OPEN[phm->wAdapterIndex][phm->u.d.wStreamIndex],
						sizeof(aRESP_HPI_OSTREAM_OPEN[0][0]));
		goto exit;
	} else {
		aOStreamUserOpen[phm->wAdapterIndex][phm->u.d.wStreamIndex].nOpenFlag = 1;
		HpiOs_Msgxlock_UnLock(&msgxLock, &flags);
		...
		HpiOs_Msgxlock_Lock(&msgxLock, &flags);
		if ( hr.wError ) {
			aOStreamUserOpen[phm->wAdapterIndex][phm->u.d.wStreamIndex].nOpenFlag = 0;
			phr->wError = hr.wError;
		} else {
			aOStreamUserOpen[phm->wAdapterIndex][phm->u.d.wStreamIndex].nOpenFlag = 1;
			aOStreamUserOpen[phm->wAdapterIndex][phm->u.d.wStreamIndex].hOwner=hOwner;
			memcpy( phr, &aRESP_HPI_OSTREAM_OPEN[phm->wAdapterIndex][phm->u.d.wStreamIndex],
                        sizeof(aRESP_HPI_OSTREAM_OPEN[0][0]));
		}
	}
...
static void OutStreamClose( HPI_MESSAGE *phm, HPI_RESPONSE *phr, void * hOwner ) {
	...
	HpiOs_Msgxlock_Lock(&msgxLock, &flags);
	if ( hOwner==aOStreamUserOpen[phm->wAdapterIndex][phm->u.d.wStreamIndex].hOwner ) {
		aOStreamUserOpen[phm->wAdapterIndex][phm->u.d.wStreamIndex].hOwner = NULL;
		HpiOs_Msgxlock_UnLock(&msgxLock, &flags);
		// issue a reset
		memcpy( &hm, phm, sizeof(HPI_MESSAGE));
		hm.wFunction = HPI_OSTREAM_RESET;
		...
		HPI_DEBUG_LOG2(WARNING,"%p trying to close outstream owned by %p\n",
			hOwner,
			aOStreamUserOpen[phm->wAdapterIndex][phm->u.d.wStreamIndex].hOwner);
		phr->wError =  HPI_ERROR_OBJ_NOT_OPEN;
	}
	HpiOs_Msgxlock_UnLock(&msgxLock, &flags);
}

static HW16 AdapterPrepare(
    HW16 wFoundAdapter,
    HW16 isPnP
)
{
    ...
    for(wAdapter = (isPnP?wFoundAdapter:0) ; wAdapter < HPI_MAX_ADAPTERS ; wAdapter++ )
    {
        HW16 i;

        if(gRESP_HPI_SUBSYS_FIND_ADAPTERS.s.awAdapterList[wAdapter]==0 && !isPnP)
            continue;
        ...
        HW_EntryPoint( &hm, &hr );
        memcpy(&aRESP_HPI_ADAPTER_OPEN[wAdapter], &hr, sizeof(aRESP_HPI_ADAPTER_OPEN[0]) );
        if(hr.wError)
        {
            if (!isPnP)
                continue;
        ...
        if(isPnP)
        {
            ...
            if(gRESP_HPI_SUBSYS_FIND_ADAPTERS.s.wNumAdapters>HPI_MAX_ADAPTERS)
                gRESP_HPI_SUBSYS_FIND_ADAPTERS.s.wNumAdapters = HPI_MAX_ADAPTERS;
        }
        ...
        for(i=0; i<aADAPTER_INFO[wAdapter].wNumOutStreams; i++)
        {
            ...
            HW_EntryPoint( &hm, &hr );
            memcpy(&aRESP_HPI_OSTREAM_OPEN[wAdapter][i], &hr, sizeof(aRESP_HPI_OSTREAM_OPEN[0][0]) );
        ...
        for(i=0; i<aADAPTER_INFO[wAdapter].wNumInStreams; i++)
        {
            ...
            HW_EntryPoint( &hm, &hr );
            memcpy(&aRESP_HPI_ISTREAM_OPEN[wAdapter][i], &hr, sizeof(aRESP_HPI_ISTREAM_OPEN[0][0]) );
        ...
        hm.wAdapterIndex = wAdapter;
        HW_EntryPoint( &hm, &hr );
        memcpy(&aRESP_HPI_MIXER_OPEN[wAdapter], &hr, sizeof(aRESP_HPI_MIXER_OPEN[0]) );
   ...
   // Check each supported PCI device ID in turn.
	for (i=0; i< (sizeof(pci_dev_tbl)/sizeof(pci_dev_tbl[0])); i++) {
			...
			)
		{
			...
			if( hr.wError ) {
				phr->wError = nError;
				HPI_DEBUG_LOG1(ERROR,"%d from CreateAdapterObj\n",nError);
				continue;
			}
			...
			if( phr->u.s.awAdapterList[ hr.u.s.wAdapterIndex ] != 0) {
				...
				HPI_DEBUG_LOG3(ERROR,"Existing ASI%4x index %d, can't add this ASI%4x\n",
				phr->u.s.awAdapterList[hr.u.s.wAdapterIndex],
				hr.u.s.wAdapterIndex, hr.u.s.awAdapterList[hr.u.s.wAdapterIndex] );
				continue;
			}

			hpi_entry_points[ hr.u.s.wAdapterIndex ] = entry_point_func;

			phr->u.s.awAdapterList[ hr.u.s.wAdapterIndex ] = hr.u.s.awAdapterList[hr.u.s.wAdapterIndex];
...
static void HPIMSGX_Reset( HW16 wAdapterIndex ) {
    ...
    if(wAdapterIndex==HPIMSGX_ALLADAPTERS)
    {
		...
		HPI_InitResponse(&hr, HPI_OBJ_SUBSYSTEM, HPI_SUBSYS_FIND_ADAPTERS,0 );
		memcpy(&gRESP_HPI_SUBSYS_FIND_ADAPTERS,&hr,sizeof(&gRESP_HPI_SUBSYS_FIND_ADAPTERS));

        for(wAdapter=0;wAdapter<HPI_MAX_ADAPTERS; wAdapter++)
        {
			HPI_InitResponse(&hr, HPI_OBJ_ADAPTER, HPI_ADAPTER_OPEN,HPI_ERROR_BAD_ADAPTER );
			memcpy(&aRESP_HPI_ADAPTER_OPEN[wAdapter],&hr,sizeof(aRESP_HPI_ADAPTER_OPEN[wAdapter]));

			HPI_InitResponse(&hr, HPI_OBJ_MIXER, HPI_MIXER_OPEN,HPI_ERROR_INVALID_OBJ);
			memcpy(&aRESP_HPI_MIXER_OPEN[wAdapter],&hr,sizeof(aRESP_HPI_MIXER_OPEN[wAdapter]));

            for(i=0;i<HPI_MAX_STREAMS;i++)
            {
				HPI_InitResponse(&hr, HPI_OBJ_OSTREAM, HPI_OSTREAM_OPEN,HPI_ERROR_INVALID_OBJ);
				memcpy(&aRESP_HPI_OSTREAM_OPEN[wAdapter][i],&hr,sizeof(aRESP_HPI_OSTREAM_OPEN[wAdapter][i]));
				HPI_InitResponse(&hr, HPI_OBJ_ISTREAM, HPI_ISTREAM_OPEN,HPI_ERROR_INVALID_OBJ);
				memcpy(&aRESP_HPI_ISTREAM_OPEN[wAdapter][i],&hr,sizeof(aRESP_HPI_ISTREAM_OPEN[wAdapter][i]));
			}
        }
    }
    else if(wAdapterIndex<HPI_MAX_ADAPTERS)
    {
        aRESP_HPI_ADAPTER_OPEN[wAdapterIndex].h.wError = HPI_ERROR_BAD_ADAPTER;
        aRESP_HPI_MIXER_OPEN[wAdapterIndex].h.wError = HPI_ERROR_INVALID_OBJ;
        for(i=0;i<HPI_MAX_STREAMS;i++)
        {
            aRESP_HPI_OSTREAM_OPEN[wAdapterIndex][i].h.wError = HPI_ERROR_INVALID_OBJ;
            aRESP_HPI_ISTREAM_OPEN[wAdapterIndex][i].h.wError = HPI_ERROR_INVALID_OBJ;
        }
        if(gRESP_HPI_SUBSYS_FIND_ADAPTERS.s.awAdapterList[wAdapterIndex])
        {
            gRESP_HPI_SUBSYS_FIND_ADAPTERS.s.awAdapterList[wAdapterIndex] = 0;
            gRESP_HPI_SUBSYS_FIND_ADAPTERS.s.wNumAdapters--;
        }
    }
}

static HW16 HPIMSGX_Init(
...
)
{
    ...
    if(phm)				// If PnP OS finds an adapter and add it
    {
        ...
        HPI_InitResponse(&hr, phm->wObject, phm->wFunction, HPI_ERROR_INVALID_OBJ );
		memcpy(hr.u.s.awAdapterList, gRESP_HPI_SUBSYS_FIND_ADAPTERS.s.awAdapterList, sizeof(gRESP_HPI_SUBSYS_FIND_ADAPTERS.s.awAdapterList) );

			entry_point_func = HPI_LookupEntryPointFunction( phm->u.s.Resource.r.Pci );
			...
			if ( hr.wError == 0 ) {
				...
				HPI_DEBUG_LOG0(DEBUG,"HPI_SUBSYS_CREATE_ADAPTER successful, preparing adapter\n");
	...
	//	printk(KERN_INFO "Cleanup adapter #%d\n",wAdapter);
        ...
        for(i=0; i<HPI_MAX_STREAMS; i++)
        {
            if( hOwner == aOStreamUserOpen[wAdapter][i].hOwner )
            {
                ...
                HPI_DEBUG_LOG2(DEBUG,"Close adapter %d ostream %d\n",wAdapter,i);

                HPI_InitMessage( &hm, HPI_OBJ_OSTREAM, HPI_OSTREAM_RESET );
                hm.wAdapterIndex = (HW16)wAdapter;
                hm.u.d.wStreamIndex = (HW16)i;
            ...
            if( hOwner == aIStreamUserOpen[wAdapter][i].hOwner )
            {
                ...
                HPI_DEBUG_LOG2(DEBUG,"Close adapter %d istream %d\n",wAdapter,i);
