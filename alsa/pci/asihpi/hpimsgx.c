...
HW16 HPI_AdapterFindObject( const HPI_HSUBSYS *phSubSys, HW16 wAdapterIndex, HW16 wObjectType, HW16 wObjectIndex,HW16 *pDspIndex);
...
)
{
    ...
    if(wAdapterIndex==HPIMSGX_ALLADAPTERS)
    {
        ...
        {
            aRESP_HPI_ADAPTER_OPEN[wAdapter].h.wError = HPI_ERROR_BAD_ADAPTER;
            aRESP_HPI_MIXER_OPEN[wAdapter].h.wError = HPI_ERROR_INVALID_OBJ;
            for(i=0;i<HPI_MAX_STREAMS;i++)
            {
                aRESP_HPI_OSTREAM_OPEN[wAdapter][i].h.wError = HPI_ERROR_INVALID_OBJ;
                aRESP_HPI_ISTREAM_OPEN[wAdapter][i].h.wError = HPI_ERROR_INVALID_OBJ;
            }
        }
    }
...
)
{
    ...
    {
        HW16 wNumInStreams,wNumOutStreams,i;
        ...
        {
            HPI_AdapterFindObject( phSubSysHandle, wAdapter, HPI_OBJ_ISTREAM, i,
...
)
{
    ...
    if(hm.wAdapterIndex>=HPI_MAX_ADAPTERS)
    {
        ...
        memcpy( phr, &aRESP_HPI_ADAPTER_OPEN[0], sizeof(aRESP_HPI_ADAPTER_OPEN[0]));
    ...
    else
    {
		...
		case HPI_SUBSYS_CLOSE:
			...
			hr.wError = 0;
			memcpy( phr, &hr,hr.wSize);
			HPIMSGX_MessageExAdapterCleanup(HPIMSGX_ALLADAPTERS,hOwner);
			break;
        ...
        case HPI_ADAPTER_OPEN:
            ...
            memcpy( phr, &aRESP_HPI_ADAPTER_OPEN[hm.wAdapterIndex], sizeof(aRESP_HPI_ADAPTER_OPEN[0]));
            break;
        case HPI_MIXER_OPEN:
            // send response back to user mode
            memcpy( phr, &aRESP_HPI_MIXER_OPEN[hm.wAdapterIndex], sizeof(aRESP_HPI_MIXER_OPEN[0]));
            break;
        case HPI_OSTREAM_OPEN:
            {
                // send response back to user mode
                memcpy( phr, &aRESP_HPI_OSTREAM_OPEN[hm.wAdapterIndex][hm.u.d.wOStreamIndex],
                        sizeof(aRESP_HPI_OSTREAM_OPEN[0][0]));
                if(aOStreamUserOpen[hm.wAdapterIndex][hm.u.d.wOStreamIndex].nOpenFlag) {
		    ...
		    if (hr.wError)
                    ...
                    {
                        aOStreamUserOpen[hm.wAdapterIndex][hm.u.d.wOStreamIndex].nOpenFlag=1;
                        aOStreamUserOpen[hm.wAdapterIndex][hm.u.d.wOStreamIndex].hOwner=hOwner;
                    }
                }
            }
            break;
        case HPI_OSTREAM_CLOSE:
            {
		if (hOwner==aOStreamUserOpen[hm.wAdapterIndex][hm.u.d.wOStreamIndex].hOwner) {
		    // issue a reset
		    hm.wFunction=HPI_OSTREAM_RESET;
		    HPI_Message(phSubSys,&hm,&hr);
			...
			{
			    hr.wSize = sizeof(HPI_STREAM_RESPONSE);
			    hr.wType = HPI_TYPE_RESPONSE;
			    hr.wObject = hm.wObject;
			    hr.wFunction = hm.wFunction;
			    hr.wError = 0;
			    aOStreamUserOpen[hm.wAdapterIndex][hm.u.d.wOStreamIndex].nOpenFlag=0;
			    aOStreamUserOpen[hm.wAdapterIndex][hm.u.d.wOStreamIndex].hOwner=0;
			    memcpy( phr, &hr,hr.wSize);
			}
		} else
        ...
        case HPI_ISTREAM_OPEN:
            {
                // send response back to user mode
                memcpy( phr, &aRESP_HPI_ISTREAM_OPEN[hm.wAdapterIndex][hm.u.d.wIStreamIndex],
                        sizeof(aRESP_HPI_ISTREAM_OPEN[0][0]));

                if(aIStreamUserOpen[hm.wAdapterIndex][hm.u.d.wIStreamIndex].nOpenFlag)
                    phr->wError = HPI_ERROR_OBJ_ALREADY_OPEN;
                else
                {
                    // issue a reset
		    ...
		    if (hr.wError)
                    ...
                    {
                        aIStreamUserOpen[hm.wAdapterIndex][hm.u.d.wIStreamIndex].nOpenFlag=1;
                        aIStreamUserOpen[hm.wAdapterIndex][hm.u.d.wIStreamIndex].hOwner=hOwner;
                    }
                }
            }
            break;
        case HPI_ISTREAM_CLOSE:
            {
		if (hOwner==aIStreamUserOpen[hm.wAdapterIndex][hm.u.d.wIStreamIndex].hOwner) {
		    // issue a reset
		    hm.wFunction=HPI_ISTREAM_RESET;
		    HPI_Message(phSubSys,&hm,&hr);
		    ...
		    else
			{
			    ...
			    hr.wFunction = hm.wFunction;
			    hr.wError = 0;
			    aIStreamUserOpen[hm.wAdapterIndex][hm.u.d.wIStreamIndex].nOpenFlag=0;
			    aIStreamUserOpen[hm.wAdapterIndex][hm.u.d.wIStreamIndex].hOwner=0;
			    memcpy( phr, &hr,  hr.wSize);
			}
		}
            }
        ...
        case HPI_ADAPTER_CLOSE:
            {
		HPIMSGX_MessageExAdapterCleanup(hm.wAdapterIndex,hOwner);
        ...
        default:
            {
                ...
                memcpy( phr, &hr, /* sizeof(hr)*/ hr.wSize);
            }
            break;
        } // switch ( wFunction )
    } // else
	...
	//	printk(KERN_INFO "Cleanup adapter #%d\n",wAdapter);
        ...
        for(i=0; i<HPI_MAX_STREAMS; i++)
        {
            if( hOwner == aOStreamUserOpen[wAdapter][i].hOwner )
            {
                ...
                HPI_PRINT_VERBOSE("Close adapter %d ostream %d\n",wAdapter,i);

			    HPI_InitMessage( &hm,  HPI_OBJ_OSTREAM, HPI_OSTREAM_HOSTBUFFER_FREE );
			    hm.wAdapterIndex = (HW16)wAdapter;
			    hm.u.d.wOStreamIndex = (HW16)i;
			    hm.wDspIndex = aOStreamUserOpen[wAdapter][i].wDspIndex;
			    HPI_Message(0, &hm, &hr );
            ...
            if( hOwner == aIStreamUserOpen[wAdapter][i].hOwner )
            {
                ...
                HPI_PRINT_VERBOSE("Close adapter %d istream %d\n",wAdapter,i);

			    HPI_InitMessage( &hm,  HPI_OBJ_ISTREAM, HPI_ISTREAM_HOSTBUFFER_FREE );
			    hm.wAdapterIndex = (HW16)wAdapter;
			    hm.u.d.wIStreamIndex = (HW16)i;
			    hm.wDspIndex = aIStreamUserOpen[wAdapter][i].wDspIndex;
			    HPI_Message(0, &hm, &hr );
