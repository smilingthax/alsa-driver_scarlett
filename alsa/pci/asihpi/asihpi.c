					    ...
					    substream)
{
	...
	dpcm->timer.expires = jiffies + dpcm->pcm_jiffie_per_period + dpcm->pcm_jiffie_per_period/4;
...
static int snd_card_asihpi_playback_hw_params(struct snd_pcm_substream * substream,
					      struct snd_pcm_hw_params *params)
{
	struct snd_pcm_runtime *runtime = substream->runtime;
	snd_card_asihpi_pcm_t *dpcm = runtime->private_data;
...
static void snd_card_asihpi_playback_format(snd_card_asihpi_t *asihpi,HPI_HOSTREAM hStream,
					    u64 * formats)
{
					 ...
					 snd_pcm_uframes_t count)
{
	...
	err = HPI_OutStreamWriteBuf(phSubSys, dpcm->hStream, runtime->dma_area, len, &dpcm->Format);
	HPI_HandleError(err);
					     ...
					     struct snd_pcm_hw_params *params)
{
	...
	if (enable_bbm) {
		...
		err = HPI_StreamEstimateBufferSize( &dpcm->Format, (HW32)( 1000 / HZ ), &minBufSize );
		snd_printd(KERN_INFO "InStreamHostBuffer estimated size: %d bytes\n", (int)minBufSize);
		...
		if ( err == 0 ) {
			...
			if ( err == 0 ) {
				...
				snd_printd(KERN_INFO "InStreamHostBufferAllocate error(%d)\n", err);
				return err;
			}
		} else {
...
static void snd_card_asihpi_capture_format(snd_card_asihpi_t *asihpi,HPI_HOSTREAM hStream,
					   u64 * formats)
{
  HPI_FORMAT hpi_format;
	HW16 wFormat;
...
#endif
	err = HPI_InStreamReadBuf(phSubSys, dpcm->hStream, runtime->dma_area, dwDataSize );
	HPI_HandleError(err);

	/* Used by capture_pointer */
	dpcm->pcm_irq_pos =
	    (dpcm->pcm_irq_pos + dwDataSize) % dpcm->pcm_size;
				...
				struct snd_ctl_elem_value *ucontrol)
{
	...
	for (i = 0; i < HPI_MAX_CHANNELS; i++) {
		...
		if (err) {
			...
			ucontrol->value.integer.value[i] = anGain0_01dB[i] << 16;
		} else {
			/* -ve is log value in millibels < -60dB, convert to (roughly!) linear, */
			ucontrol->value.integer.value[i]=log2lin[anGain0_01dB[i]/-1000];
		}
	...
	/* need fixed point log calculation convert 32..32767 to -60.00..0.00dB*/
		...
		anGain0_01dB[i] = (short)((float)(20*log10((float)nLinear/32767.0))*100.0);
		// units are 0.01dB
	#endif
...
static int snd_asihpi_clksrc_info(struct snd_kcontrol *kcontrol,
				  struct snd_ctl_elem_info *uinfo)
{
	snd_card_asihpi_t * asihpi=(snd_card_asihpi_t *)(kcontrol->private_data);
...
static void __devinit snd_asihpi_clksrc_new(snd_card_asihpi_t * asihpi, hpi_control_t * asihpi_control,
					 struct snd_kcontrol_new * snd_control)
{
...

{
	...
	if (hasAesIn)
		...
		for (j=1; j< 8; j++) {
			...
			clkcache->s[i].name=
				sampleclock_sources[j+HPI_SAMPLECLOCK_SOURCE_LAST];
			i++;
		}
	clkcache->count=i;
}
