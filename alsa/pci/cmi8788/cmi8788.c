...
static int   boot_devs=0;

module_param_array(index, int, NULL/*boot_devs*/, 0444);
MODULE_PARM_DESC(index, "Index value for C-Media PCI soundcard.");
module_param_array(id, charp, NULL/*boot_devs*/, 0444);
MODULE_PARM_DESC(id, "ID string for C-Media PCI soundcard.");
module_param_array(enable, bool, NULL/*boot_devs*/, 0444);
...
#define	VOL_SCALE	65536.0	// convert float to integer
#define	VOL_MAX		65536

static long HwVolTable[] =
{
    (long)(VOL_SCALE * 1.0),        /* 0     dB = 10 ^0 */
    (long)(VOL_SCALE * 0.841395),   /* -1.5  dB = 10^-0.075 */
    (long)(VOL_SCALE * 0.707946),   /* -3    dB = 10^-0.15 */
    (long)(VOL_SCALE * 0.595662),   /* -4.5  dB = 10^-0.225 */
    (long)(VOL_SCALE * 0.501187),   /* -6    dB = 10^-0.3 */
    (long)(VOL_SCALE * 0.421697),   /* -7.5  dB = 10^-0.375 */
    (long)(VOL_SCALE * 0.354813),   /* -9    dB = 10^-0.45 */
    (long)(VOL_SCALE * 0.298538),   /* -10.5 dB = 10^-0.525 */
    (long)(VOL_SCALE * 0.251189),   /* -12   dB = 10^-0.6 */
    (long)(VOL_SCALE * 0.211349),   /* -13.5 dB = 10^-0.675 */
    (long)(VOL_SCALE * 0.177828),   /* -15   dB = 10^-0.75 */
    (long)(VOL_SCALE * 0.149624),   /* -16.5 dB = 10^-0.825 */
    (long)(VOL_SCALE * 0.125893),   /* -18   dB = 10^-0.9 */
    (long)(VOL_SCALE * 0.105925),   /* -19.5 dB = 10^-0.975 */
    (long)(VOL_SCALE * 0.0891251),  /* -21   dB = 10^-1.05 */
    (long)(VOL_SCALE * 0.0749894),  /* -22.5 dB = 10^-1.125 */
    (long)(VOL_SCALE * 0.0630957),  /* -24   dB = 10 ^1.2 */
    (long)(VOL_SCALE * 0.0530884),  /* -25.5 dB = 10^-1.275 */
    (long)(VOL_SCALE * 0.0446684),  /* -27   dB = 10^-1.35 */
    (long)(VOL_SCALE * 0.0375837),  /* -28.5 dB = 10^-1.425 */
    (long)(VOL_SCALE * 0.0316228),  /* -30   dB = 10^-1.5 */
    (long)(VOL_SCALE * 0.0266073),  /* -31.5 dB = 10^-1.575 */
    (long)(VOL_SCALE * 0.0223872),  /* -33   dB = 10^-1.65 */
    (long)(VOL_SCALE * 0.0188365),  /* -34.5 dB = 10^-1.725 */
    (long)(VOL_SCALE * 0.0158489),  /* -36   dB = 10^-1.8 */
    (long)(VOL_SCALE * 0.0133352),  /* -37.5 dB = 10^-1.875 */
    (long)(VOL_SCALE * 0.0112202),  /* -39   dB = 10^-1.95 */
    (long)(VOL_SCALE * 0.00944061), /* -40.5 dB = 10^-2.025 */
    (long)(VOL_SCALE * 0.00794328), /* -42   dB = 10^-2.1 */
    (long)(VOL_SCALE * 0.00668344), /* -43.5 dB = 10^-2.175 */
    (long)(VOL_SCALE * 0.00562341), /* -45   dB = 10^-2.25 */
    (long)(VOL_SCALE * 0.00421697)  /* -46.5 dB = 10^-2.325 */
};

extern int __devinit snd_cmi8788_mixer_create(snd_cmi8788 *chip);

/* read/write operations for dword register */
/*inline*/ static void snd_cmipci_write(snd_cmi8788 *chip, unsigned int data, unsigned int cmd)
{

//cmi_printk((" -- snd_cmipci_write\n"));
//    return;
    outl(data, chip->addr + cmd);
}
 static unsigned int snd_cmipci_read(snd_cmi8788 *chip, unsigned int cmd)
{
//cmi_printk((" -- snd_cmipci_read\n"));
//    return 0;
...
static void snd_cmipci_write_w(snd_cmi8788 *chip, unsigned short data, unsigned int cmd)
{
//cmi_printk((" -- snd_cmipci_write_w\n"));
//    return;

    outw(data, chip->addr + cmd);
}
static unsigned short snd_cmipci_read_w(snd_cmi8788 *chip, unsigned int cmd)
{
//cmi_printk((" -- snd_cmipci_read_w\n"));
//    return 0;
...
static void snd_cmipci_write_b(snd_cmi8788 *chip, unsigned char data, unsigned int cmd)
{
//cmi_printk((" -- snd_cmipci_write_b\n"));
//    return;

    outb(data, chip->addr + cmd);
}

 static unsigned char snd_cmipci_read_b(snd_cmi8788 *chip, unsigned int cmd)
{
//cmi_printk((" -- snd_cmipci_read_b\n"));
//    return 0;
...
static int snd_cmi_send_spi_cmd(cmi_codec *codec, u8 *data)
{
    ...
    snd_cmi8788 *chip = (snd_cmi8788 *)codec->controller->private_data;

    if(0 == codec->reg_len_flag) // 2bytes
    {
    ...
    else if(1 == codec->reg_len_flag) // 3bytes
    {
        snd_cmipci_write_b(chip, data[0], SPI_Data+0); // byte
        snd_cmipci_write_b(chip, data[1], SPI_Data+1); // byte
...
{
//    snd_cmi8788 *chip = codec->bus->private_data;
...
static struct snd_pcm_hardware snd_cmi_pcm_playback_hw = {
    ...
    .rates =        SNDRV_PCM_RATE_5512 | SNDRV_PCM_RATE_8000_48000 /*SNDRV_PCM_RATE_48000*/,
...
{
    snd_cmi8788 *chip          = (snd_cmi8788 *)snd_pcm_substream_chip(substream);
...
static int snd_cmi_pcm_playback_prepare(struct snd_pcm_substream *substream)
{
    ...
    int                 codec_num  = controller->codec_num;
...
//    snd_cmipci_write(chip, (cmi_subs->fragsize)/4-1 , PCI_DMAPlay_MUTLI_BaseTCount);// d-word units
    ...
    val16 = 0;
    u8  fmt   = snd_cmipci_read_b(chip, PCI_PlaySampleFmCvt);
    u16 I2SFmt= snd_cmipci_read_w(chip, I2S_Multi_DAC_Fmt);
    ...
    {
    case 16:
        fmt &= 0xf3; fmt |= 0x00; //Bit 3:2 00
        I2SFmt &= 0xff3f; I2SFmt |= 0x0000;//Bit 7:6  00

        break;
    case 20:
        I2SFmt &= 0xff3f; I2SFmt |= 0x0040;//Bit 7:6  01
        break;
    case 24:
        fmt &= 0xf3; fmt |= 0xf4; //Bit 3:2 01
        I2SFmt &= 0xff3f; I2SFmt |= 0x0080;//Bit 7:6  10
        break;
    case 32:
        fmt &= 0xf3; fmt |= 0xf8; //Bit 3:2 10

        I2SFmt &= 0xff3f; I2SFmt |= 0x00c0;//Bit 7:6  11
        break;
    }
 ...
 // Multi-Channel DMA Mode
    u8  PlyDmaMode = snd_cmipci_read_b(chip, PCI_MULTI_DMA_MODE);
    cmi_printk(("  PCI_MULTI_DMA_MODE Reg0x%0x, Read-1 0x%0x\n", PCI_MULTI_DMA_MODE, PlyDmaMode));
//  cmi_printk(("  codec addr 0x%0x\n", codec));
    if(codec)
    {
    ...
    case 2:
        // Bit 1:0  00
        PlyDmaMode &= 0xfc; PlyDmaMode |= 0x00;
        codec[0].pcm_substream[0].ops.prepare((void *)(&codec[0].pcm_substream[0]), &codec[0], substream);
        break;
    case 4:

        // Bit 1:0  01
        PlyDmaMode &= 0xfc; PlyDmaMode |= 0x01;
        codec[0].pcm_substream[0].ops.prepare((void *)(&codec[0].pcm_substream[0]), &codec[0], substream);
        codec[1].pcm_substream[0].ops.prepare((void *)(&codec[1].pcm_substream[0]), &codec[1], substream);       
        break;
    case 6:
        // Bit 1:0  10
        PlyDmaMode &= 0xfc; PlyDmaMode |= 0x02;
        codec[0].pcm_substream[0].ops.prepare((void *)(&codec[0].pcm_substream[0]), &codec[0], substream);

        codec[1].pcm_substream[0].ops.prepare((void *)(&codec[1].pcm_substream[0]), &codec[1], substream);
        codec[2].pcm_substream[0].ops.prepare((void *)(&codec[2].pcm_substream[0]), &codec[2], substream);
        break;
    case 8:
        // Bit 1:0  11
        PlyDmaMode &= 0xfc; PlyDmaMode |= 0x03;
        codec[0].pcm_substream[0].ops.prepare((void *)(&codec[0].pcm_substream[0]), &codec[0], substream);
        codec[1].pcm_substream[0].ops.prepare((void *)(&codec[1].pcm_substream[0]), &codec[1], substream);
        codec[2].pcm_substream[0].ops.prepare((void *)(&codec[2].pcm_substream[0]), &codec[2], substream);
        codec[3].pcm_substream[0].ops.prepare((void *)(&codec[3].pcm_substream[0]), &codec[3], substream);
        break;
    default:
        break;
    }
...
//    snd_cmipci_write_w(chip, (cmi_subs->dma_bytes/8)-1 ,PCI_DMARec_A_BaseTCount); // d-word units
    ...
    case 20:
        ...
        break;
    case 24:
        fmt &= 0xfc; fmt |= 0x01;//Bit 1:0 01
        I2SFmt &= 0xff3f ; I2SFmt |= 0x0080;//Bit 7:6 10
        break;
    case 32:
        fmt &= 0xfc; fmt |= 0x02;//Bit 1:0 10
        I2SFmt &= 0xff3f ; I2SFmt |= 0x00c0;//Bit 7:6 11
    ...
    u8 RecDmaMode = snd_cmipci_read_b(chip, PCI_RecDMA_Mode);
    ...
    case 2:
        ...
        RecDmaMode &= 0xf8; RecDmaMode |= 0x00;
        break;
    case 4:
        // Bit 2:0 001
        RecDmaMode &= 0xf8; RecDmaMode |= 0x01;
        break;
    case 6:
        // Bit 2:0 011
        RecDmaMode &= 0xf8; RecDmaMode |= 0x03; // or 0x02
        break;
    case 8:
        // Bit 2:0 100
        RecDmaMode &= 0xf8; RecDmaMode |= 0x04;
        break;
    default:
        break;
...
//  cmi_printk((" playback_pointer, addr 0x%0x, pos %d\n", addr, pos));
...
//  cmi_printk((" ac97_playback_pointer, addr 0x%0x, pos %d\n", addr, pos));
...
static irqreturn_t snd_cmi8788_interrupt(int irq, void *dev_id)
{
    ...
    //cmi_printk(("snd_cmi8788_interrupt\n"));
    ...
    for(i=0; i<chip->PCM_Count; i++)
    {
        ...
        if(cmi_subs->running)
        {
            if(status & cmi_subs->int_sta_mask)
            {
                ...
                // disable interrupt
                int_val = old_int_val & (~(cmi_subs->int_mask));// 0xffef ==  ~0x0010
                snd_cmipci_write_w(chip, int_val, PCI_IntMask);
            ...
            //  cmi_printk(("playback interrupt(%i64) %d\n", jiffies, jiffies - chip->old_jiffies));
        ...
        if(cmi_subs->running)
        {
            if(status & cmi_subs->int_sta_mask)
            {
                ...
                // disable interrupt
                int_val = old_int_val & (~(cmi_subs->int_mask));// 0xfffe == ~0x0001; // clear Bit-0
                snd_cmipci_write_w(chip, int_val, PCI_IntMask);
...
static int __devinit snd_cmi8788_pcm_create(snd_cmi8788 *chip)
{
...
#if 1 // swf 2005-04-25        
    // 1 create normal PCM
    err = snd_pcm_new(chip->card, "C-Media PCI8788 DAC/ADC"/*chip->card->driver*/, pcm_dev, 1, 1, &chip->pcm[pcm_dev]);
    if (err < 0)
        return err;
    ...
    // 2 create AC97 PCM
    err = snd_pcm_new(chip->card, "C-Media PCI8788 DAC/ADC"/*chip->card->driver*/, pcm_dev, 1, 0, &chip->pcm[pcm_dev]);
    if (err < 0)
        return err;
...
static int __devinit snd_cmi8788_create(struct snd_card *card, struct pci_dev *pci, snd_cmi8788 **rchip)
{
    ...
    *rchip = NULL;
    ...
    chip = kcalloc(1, sizeof(snd_cmi8788), GFP_KERNEL);
    ...
    memset(chip, 0 ,sizeof(snd_cmi8788));
...
//    init_MUTEX(&chip->open_mutex);
    ...
    if ((err = pci_request_regions(pci, chip->card->driver)) < 0) {// "C-MEDIA CMI8788")) < 0) {
        kfree(chip);
        pci_disable_device(pci);
        return err;
    }
...
*/    
    if (request_irq(pci->irq, snd_cmi8788_interrupt, SA_INTERRUPT|SA_SHIRQ, chip->card->driver , (void*)chip)) //"C-MEDIA CMI8788", (void*)chip))
    {
...
static int __devinit snd_cmi8788_probe(struct pci_dev *pci, const struct pci_device_id *pci_id)
{
    ...
    switch (pci->device) {
    case PCI_DEVICE_ID_CMEDIA_CM8788:
...
/*
    ...
    if ((err = snd_device_new(card, SNDRV_DEV_LOWLEVEL, chip, &ops)) <0) {
        snd_printk(KERN_ERR "Error creating device [card]!\n");
        snd_cmi8788_free(chip);
...
static struct pci_device_id snd_cmi8788_ids[] = {
    {PCI_VENDOR_ID_CMEDIA, PCI_DEVICE_ID_CMEDIA_CM8788, PCI_ANY_ID, PCI_ANY_ID, 0, 0, 0},
