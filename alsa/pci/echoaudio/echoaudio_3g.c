...
static int check_asic_status(struct echoaudio *chip)
{
	...
	chip->comm_page->ext_box_status = __constant_cpu_to_le32(E3G_ASIC_NOT_LOADED);
...
static int set_digital_mode(struct echoaudio *chip, u8 mode)
{
	...
	/* If we successfully changed the digital mode from or to ADAT, then make sure
	all output, input and monitor levels are updated by the DSP comm object. */
...
static int load_asic(struct echoaudio *chip)
{
	...
	err = load_asic_generic(chip, DSP_FNC_LOAD_3G_ASIC, &card_fw[FW_3G_ASIC]);
	if (err < 0)
		return err;
...
static int set_input_clock(struct echoaudio *chip, u16 clock)
{
	...
	control_reg = le32_to_cpu(chip->comm_page->control_register) & E3G_CLOCK_CLEAR_MASK;
	clocks_from_dsp = le32_to_cpu(chip->comm_page->status_clocks);
