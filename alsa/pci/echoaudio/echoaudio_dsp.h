...
typedef struct {					//					Base	Length
	...
	s8 line_out_level[DSP_MAXAUDIOOUTPUTS];		// Output gain				0x220	16*1
	s8 line_in_level[DSP_MAXAUDIOINPUTS];		// Input gain				0x230	16*1
	s8 monitors[MONITOR_ARRAY_SIZE];		// Monitor map				0x240	0x180
	u32 play_coeff[MAX_PLAY_TAPS];			// Gina/Darla play filters - obsolete	0x3c0	168*4
	u32 rec_coeff[MAX_REC_TAPS];			// Gina/Darla record filters - obsolete	0x660	192*4
	volatile u16 midi_input[MIDI_IN_BUFFER_SIZE];	// MIDI input data transfer buffer	0x960	256*2
	...
	volatile u32 status_clocks;			// Current Input clock state		0xb6c	4
	u32 ext_box_status;				// External box status			0xb70	4
	u32 cmd_add_buffer;				// Pipes to add (obsolete)		0xb74	4
	volatile u32 midi_out_free_count;		// # of bytes free in MIDI output FIFO	0xb78	4
	u32 unused2;					// Cyclic pipes				0xb7c	4
	u32 control_register;				// Mona, Gina24, Layla24, 3G ctrl reg	0xb80	4
