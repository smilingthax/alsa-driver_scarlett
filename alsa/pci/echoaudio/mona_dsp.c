...
static int load_asic(struct echoaudio *chip)
{
	...
	/* Do the external one */
	err = load_asic_generic(chip, DSP_FNC_LOAD_MONA_EXTERNAL_ASIC, &card_fw[FW_MONA_2_ASIC]);
	if (err < 0)
		return err;
...
static int set_input_clock(struct echoaudio *chip, u16 clock)
{
	...
	case ECHO_CLOCK_SPDIF:
		if (chip->digital_mode == DIGITAL_MODE_ADAT)
			return -EAGAIN;
		spin_unlock_irq(&chip->lock);
		err = switch_asic(chip, clocks_from_dsp & GML_CLOCK_DETECT_BIT_SPDIF96);
		spin_lock_irq(&chip->lock);
		if (err < 0)
			return err;
	...
	case ECHO_CLOCK_WORD:
		...
		spin_unlock_irq(&chip->lock);
		err = switch_asic(chip, clocks_from_dsp & GML_CLOCK_DETECT_BIT_WORD96);
		spin_lock_irq(&chip->lock);
		if (err < 0)
			return err;
...
static int dsp_set_digital_mode(struct echoaudio *chip, u8 mode)
{
	...
	case DIGITAL_MODE_SPDIF_RCA:
		...
		break;
	case DIGITAL_MODE_ADAT:
		/* If the current ASIC is the 96KHz ASIC, switch the ASIC and set to 48 KHz */
