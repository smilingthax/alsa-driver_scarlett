 ...
 *  and I was stupid enough to manage to get hold of a PCI168 soundcard >:-P}
				...
				int do_recording)
{
	...
	if (!chip->is_playing)
	{
		...
		count2 = (size/2)-1;
#if DEBUG_PLAY_REC
		/*printk(KERN_ERR "setting dma: addr %08lx, count %d, size %d\n", addr, count, size);*/
		printk(KERN_ERR "setting dma: buf1 %08lx[%d], buf2 %08lx[%d]\n", addr1, count1, addr2, count2);
#endif
		spin_lock_irqsave(&chip->reg_lock, flags);
...
#if DEBUG_CALLS
	printk(KERN_ERR "snd_azf3328_playback_trigger cmd %d\n", cmd);
#endif
	switch (cmd) {
...
#if DEBUG_CALLS
	printk(KERN_ERR "snd_azf3328_capture_trigger cmd %d\n", cmd);
#endif
        switch (cmd) {
        case SNDRV_PCM_TRIGGER_START:
...
static void snd_azf3328_interrupt(int irq, void *dev_id, struct pt_regs *regs)
{
	...
	if (status & IRQ_PLAYBACK)
	{
		...
		{
			printk(KERN_ERR "azt3328: unknown play IRQ type occurred, please report !\n");
		}
		if (chip->pcm && chip->playback_substream)
		{
			...
			printk(KERN_ERR "azt3328: ouch, irq handler problem !\n");
	...
	if (status & IRQ_RECORDING)
	{
		...
		{
			printk(KERN_ERR "azt3328: unknown rec IRQ type occurred, please report !\n");
		}
...
"azt3328: (bug notification, driver development etc.).\n");
	...
	printk(KERN_ERR "snd_azf3328_playback_open: %p\nsnd_azf3328_capture_open: %p\nsnd_azf3328_playback_close: %p\nsnd_azf3328_capture_close: %p\nsnd_azf3328_pcm_free: %p\nsnd_azf3328_pcm: %p\nsnd_azf3328_free: %p\nsnd_azf3328_dev_free: %p\nsnd_azf3328_create: %p\nsnd_azf3328_probe: %p\nsnd_azf3328_remove: %p\nalsa_card_azf3328_init: %p\n", snd_azf3328_playback_open, snd_azf3328_capture_open, snd_azf3328_playback_close, snd_azf3328_capture_close, snd_azf3328_pcm_free, snd_azf3328_pcm, snd_azf3328_free, snd_azf3328_dev_free, snd_azf3328_create, snd_azf3328_probe, snd_azf3328_remove, alsa_card_azf3328_init);
#endif
	snd_azf3328_dbgcallleave();
	return 0;
}
...
EXPORT_NO_SYMBOLS; /* FIXME: for older kernels */
