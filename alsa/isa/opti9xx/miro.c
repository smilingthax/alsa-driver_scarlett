...
typedef struct _snd_miro miro_t;
...
static int snd_miro_first_hit = 1;
static snd_card_t *snd_miro_legacy = SNDRV_DEFAULT_PTR1;

static void snd_miro_proc_init(miro_t * miro);
...
static int aci_busy_wait(miro_t * miro)
{
	long timeout;
	unsigned char byte;
	...
	snd_printk("aci_busy_wait() time out\n");
	return -EBUSY;
}

static inline int aci_write(miro_t * miro, unsigned char byte)
{
	if (aci_busy_wait(miro) >= 0) {
		outb(byte, miro->aci_port + ACI_REG_COMMAND);
		return 0;
	} else {
		snd_printk("aci busy, aci_write(0x%x) stopped.\n", byte);
		return -EBUSY;
	}
}

static inline int aci_read(miro_t * miro)
{
	unsigned char byte;
	...
	if (aci_busy_wait(miro) >= 0) {
		...
		snd_printk("aci busy, aci_read() stopped.\n");
		return -EBUSY;
	}
}

static int aci_cmd(miro_t * miro, int write1, int write2, int write3)
{
...
static int aci_getvalue(miro_t * miro, unsigned char index)
{
	return aci_cmd(miro, ACI_STATUS, index, -1);
}

static int aci_setvalue(miro_t * miro, unsigned char index, int value)
{
...
static int snd_miro_info_capture(snd_kcontrol_t * kcontrol,
...
static int snd_miro_get_capture(snd_kcontrol_t * kcontrol,
				snd_ctl_elem_value_t * ucontrol)
{
	miro_t *miro = snd_kcontrol_chip(kcontrol);
	int value;

	if ((value = aci_getvalue(miro, ACI_S_GENERAL)) < 0) {
		snd_printk("snd_miro_get_capture() failed: %d\n", value);
		return value;
	}
...
static int snd_miro_put_capture(snd_kcontrol_t * kcontrol,
				snd_ctl_elem_value_t * ucontrol)
{
	miro_t *miro = snd_kcontrol_chip(kcontrol);
	...
	if ((error = aci_setvalue(miro, ACI_SET_SOLOMODE, value)) < 0) {
		snd_printk("snd_miro_put_capture() failed: %d\n", error);
		return error;
	}
...
static int snd_miro_info_preamp(snd_kcontrol_t *kcontrol,
				snd_ctl_elem_info_t * uinfo)
{
...
static int snd_miro_get_preamp(snd_kcontrol_t * kcontrol,
			       snd_ctl_elem_value_t * ucontrol)
{
	miro_t *miro = snd_kcontrol_chip(kcontrol);
	int value;
	...
	if ((value = aci_getvalue(miro, ACI_GET_PREAMP)) < 0) {
		snd_printk("snd_miro_get_preamp() failed: %d\n", value);
		return value;
	}
...
static int snd_miro_put_preamp(snd_kcontrol_t * kcontrol,
			       snd_ctl_elem_value_t * ucontrol)
{
	miro_t *miro = snd_kcontrol_chip(kcontrol);
	...
	if ((error = aci_setvalue(miro, ACI_SET_PREAMP, value)) < 0) {
		snd_printk("snd_miro_put_preamp() failed: %d\n", error);
		return error;
	}
...
static int snd_miro_info_amp(snd_kcontrol_t *kcontrol,
			     snd_ctl_elem_info_t * uinfo)
{
	uinfo->type = SNDRV_CTL_ELEM_TYPE_BOOLEAN;
	uinfo->count = 1;
...
static int snd_miro_get_amp(snd_kcontrol_t * kcontrol,
			    snd_ctl_elem_value_t * ucontrol)
{
	miro_t *miro = snd_kcontrol_chip(kcontrol);
...
static int snd_miro_put_amp(snd_kcontrol_t * kcontrol,
			    snd_ctl_elem_value_t * ucontrol)
{
	miro_t *miro = snd_kcontrol_chip(kcontrol);
	int error, value, change;
	...
	if ((error = aci_setvalue(miro, ACI_SET_POWERAMP, value)) < 0) {
		snd_printk("snd_miro_put_amp() to %d failed: %d\n", value, error);
		return error;
	}
...
static int snd_miro_info_double(snd_kcontrol_t *kcontrol, 
				snd_ctl_elem_info_t * uinfo)
{
...
static int snd_miro_get_double(snd_kcontrol_t *kcontrol, 
			       snd_ctl_elem_value_t * uinfo)
{
	miro_t *miro = snd_kcontrol_chip(kcontrol);
	...
	if ((right_val = aci_getvalue(miro, right_reg)) < 0) {
		snd_printk("aci_getvalue(%d) failed: %d\n", right_reg, right_val);
		return right_val;
	}
...
static int snd_miro_put_double(snd_kcontrol_t *kcontrol, 
			       snd_ctl_elem_value_t * ucontrol)
{
	miro_t *miro = snd_kcontrol_chip(kcontrol);
...
static snd_kcontrol_new_t snd_miro_controls[] = {
...
static snd_kcontrol_new_t snd_miro_eq_controls[] = {
...
static snd_kcontrol_new_t snd_miro_radio_control[] = {
MIRO_DOUBLE("Radio Playback Volume", 0, ACI_GET_LINE1, ACI_SET_LINE1),
};

static snd_kcontrol_new_t snd_miro_line_control[] = {
MIRO_DOUBLE("Line Playback Volume", 2, ACI_GET_LINE1, ACI_SET_LINE1),
};

static snd_kcontrol_new_t snd_miro_preamp_control[] = {
{
	.iface = SNDRV_CTL_ELEM_IFACE_MIXER,
	.name = "Mic Boost",
	.index = 1,
...
static snd_kcontrol_new_t snd_miro_amp_control[] = {
{
	.iface = SNDRV_CTL_ELEM_IFACE_MIXER,
	.name = "Line Boost",
	.index = 0,
...
static snd_kcontrol_new_t snd_miro_capture_control[] = {
{
	.iface = SNDRV_CTL_ELEM_IFACE_MIXER,
...
static int __devinit snd_miro_init(miro_t *chip, unsigned short hardware)
{
	...
	default:
		snd_printk("sorry, no support for %d\n", hardware);
		return -ENODEV;
	}
...
static unsigned char snd_miro_read(miro_t *chip,
				   unsigned char reg)
{
	unsigned long flags;
...
static void snd_miro_write(miro_t *chip, unsigned char reg,
			   unsigned char value)
{
	unsigned long flags;
...
static void snd_miro_proc_read(snd_info_entry_t * entry, 
...
static int __devinit snd_miro_configure(miro_t *chip)
{
...
static int __devinit snd_card_miro_detect(snd_card_t *card, miro_t *chip)
{
	int i, err;
	unsigned char value;
...
static int __devinit snd_card_miro_aci_detect(snd_card_t *card, miro_t * miro)
{
	...
	if ((miro->res_aci_port = request_region(miro->aci_port, 3, "miro aci")) == NULL) {
		snd_printk("aci i/o area 0x%lx-0x%lx already used.\n", 
			   miro->aci_port, miro->aci_port+2);
		return -ENOMEM;
	}
	...
	for (i=0; i<3; i++)
		if (aci_cmd(miro, ACI_ERROR_OP, -1, -1) < 0) {
			snd_card_free(card);
			snd_printk("can't force aci into known state.\n");
			return -EFAULT;
	}
	...
	if ((miro->aci_vendor=aci_cmd(miro, ACI_READ_IDCODE, -1, -1)) < 0 ||
		...
		snd_card_free(card);
		snd_printk("can't read aci id on 0x%lx.\n", miro->aci_port);
		return -EFAULT;
	}

	if ((miro->aci_version=aci_cmd(miro, ACI_READ_VERSION, -1, -1)) < 0) {
		snd_card_free(card);
		snd_printk("can't read aci version on 0x%lx.\n", 
			   miro->aci_port);
		return -EFAULT;
	}
	...
	if (aci_cmd(miro, ACI_INIT, -1, -1)<0 ||
	    ...
	    aci_cmd(miro, ACI_ERROR_OP, ACI_ERROR_OP, ACI_ERROR_OP)<0) {
		snd_printk("can't initialize aci.\n"); 
		return -EFAULT;
	}
...
static void snd_card_miro_free(snd_card_t *card)
{
	miro_t *miro = (miro_t *)card->private_data;
        
	if (miro->res_aci_port) {
		release_resource(miro->res_aci_port);
		kfree_nocheck(miro->res_aci_port);
	}
	if (miro->res_mc_base) {
		release_resource(miro->res_mc_base);
		kfree_nocheck(miro->res_mc_base);
	}
	kfree(miro);
}

static int __devinit snd_card_miro_probe(void)
{
	...
	miro = (miro_t *)card->private_data;
	miro->card = card;

	if ((error = snd_card_miro_aci_detect(card, miro)) < 0) {
		snd_card_free(card);
		snd_printk("unable to detect aci chip\n");
		return -ENODEV;
	}
	...
	if ((error = snd_card_miro_detect(card, miro)) < 0) {
		snd_card_free(card);
		snd_printk("unable to detect OPTi9xx chip\n");
		return -ENODEV;
	}

	if (! miro->res_mc_base &&
	    (miro->res_mc_base = request_region(miro->mc_base, miro->mc_base_size, "miro (OPTi9xx MC)")) == NULL) {
		snd_card_free(card);
		snd_printk("request for OPTI9xx MC failed\n");
		return -ENOMEM;
	}
	...
	if (miro->aci_vendor == 'm') {
		...
		default:
			sprintf(card->shortname, 
				"unknown miro");
			snd_printk("unknown miro aci id\n");
		}
	} else {
		snd_printk("found unsupported aci card\n");
	...
	snd_miro_first_hit = 0;
	snd_miro_legacy = card;
	return 0;
}
...
static int __init alsa_card_miro_init(void)
{
	...
	if ((error = snd_card_miro_probe()) < 0) {
#ifdef MODULE
