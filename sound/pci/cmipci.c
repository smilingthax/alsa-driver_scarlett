...
{
	cmipci_t *cm = snd_pcm_substream_chip(substream);
	return snd_cmipci_pcm_trigger(cm, &cm->channel[0], substream, cmd);
}
...
{
	cmipci_t *cm = snd_pcm_substream_chip(substream);
	return snd_cmipci_pcm_trigger(cm, &cm->channel[1], substream, cmd);
}
		...
		for (i = 0; i < CM_SAVED_MIXERS; i++) {
			...
			if (ctl) {
				...
				if (val.value.integer.value[0] != cm->mixer_res_status[i]) {
					...
					ctl->put(ctl, &val);
					snd_ctl_notify(cm->card, SNDRV_CTL_EVENT_MASK_VALUE, &ctl->id);
				}
			}
		}
	}
...
static int snd_cmipci_playback_spdif_prepare(snd_pcm_substream_t *subs)
{
	...
	if (do_ac3) {
		...
		if (cm->can_ac3_hw) {
			...
			cm->channel[0].ac3_shift = 1; /* use 32bit */
		}
		...
		if (cm->can_ac3_hw)
			if (subs->runtime->format == SNDRV_PCM_FORMAT_S24_LE)
				snd_cmipci_set_bit(cm, CM_REG_CHFORMAT, CM_SPD24SEL);
			else
				snd_cmipci_clear_bit(cm, CM_REG_CHFORMAT, CM_SPD24SEL);
		else {
	...
	spin_unlock_irqrestore(&cm->reg_lock, flags);
	return snd_cmipci_pcm_prepare(cm, &cm->channel[0], subs);
}
...
static snd_cmipci_switch_args_t cmipci_switch_arg_##sname = { \
  ...
  mask_on: xmask_on, \
  is_byte: xis_byte, \
  always_accessible: xaccessible, \
}
	
#define DEFINE_BIT_SWITCH_ARG(sname, xreg, xmask, xis_byte, xaccessible) \
	DEFINE_SWITCH_ARG(sname, xreg, xmask, xmask, xis_byte, xaccessible)
...
DEFINE_BIT_SWITCH_ARG(exchange_dac, CM_REG_MISC_CTRL, CM_XCHGDAC, 0, 0);
